...
/* $Header: svn://d02584/consolrepos/branches/AR.09.03/glc/1.0.0/install/sql/XXGL_PAYROLL_PKG.pkb 2705 2017-10-05 22:52:32Z svnuser $ */
/****************************************************************************
**
** CEMLI ID: GL.02.01
**
...
BEGIN
   ...
   fnd_file.put_line(fnd_file.output,rpad('Payroll Category',45,' ')||rpad('Amount from HR',29,' ')||rpad('Journal Category',37,' ')||rpad('Journal Amount',29,' ')||rpad('Difference',20,' '));
   ...
   LOOP
         fnd_file.put_line(fnd_file.output,  rpad(r_summary.record_category, 50,' ')
                                             ...
                                             ||rpad(lpad(to_char(r_summary.Journal_maount, l_format_mask), 14, ' '),14,' ')
...
--         l_wip_category := r_summary.record_category;
         ...
         l_journal_total := l_journal_total + r_summary.Journal_maount;
         l_diferrence_total := l_diferrence_total + r_summary.diferrence;
   END LOOP;
   --IF l_catgory_total != 0 THEN
      --fnd_file.put_line(fnd_file.output,rpad(' ', 50, ' ') || lpad('-', 45, '-'));
      fnd_file.put_line(fnd_file.output,rpad(' ',40,' ')||rpad('-----------------------',31,' ')||rpad(' ',35,' ')||rpad('----------------------',32,' ')||rpad('--------------',20,' '));
   ...
   FOR r_detail IN c_detail LOOP
        ...
        IF r_detail.record_category <> nvl(l_wip_category, 'x') THEN
            debug_msg('l_wip_category :'||l_wip_category);
             fnd_file.put_line(fnd_file.output, 
                             rpad('Category : '|| r_detail.record_category,32,' ') 
                             );
             fnd_file.put_line(fnd_file.output, 
                             rpad('Flag ',10,' ') 
                            ||rpad('From',50,' ') 
                            ...
                            ||rpad('DR Amount',21,' ')
                            ||rpad('CR Amount',27,' ')
                         --||lpad(to_char(r_detail.amount, l_format_mask), 31, ' ')
                         );                             
       END IF;
       
       
       fnd_file.put_line(fnd_file.output, 
                         rpad( r_detail.flag,10,' ') 
   ...
   fnd_file.put_line(fnd_file.output, ' ');
   fnd_file.put_line(fnd_file.output, LPAD('-----------------------',129,' ')||LPAD('-----------------------',23,' '));
   fnd_file.put_line(fnd_file.output,  rpad('Total:', 90, ' ')   || lpad(to_char(l_detail_dr_total, l_format_mask), 39, ' ')|| lpad(to_char(l_detail_cr_total, l_format_mask), 20, ' '));
   fnd_file.put_line(fnd_file.output, ' ');
   fnd_file.put_line(fnd_file.output,  '                                                           **      End of Report    **            ');
