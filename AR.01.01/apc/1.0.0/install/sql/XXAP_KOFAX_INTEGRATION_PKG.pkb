...
/* $Header: svn://d02584/consolrepos/branches/AR.01.01/apc/1.0.0/install/sql/XXAP_KOFAX_INTEGRATION_PKG.pkb 3173 2017-12-12 05:28:14Z svnuser $ */
/****************************************************************************
**
** CEMLI ID: AP.03.01
**
...
/*
Note: Includes the following fixes and enhancements
...
FSC-6017: Add Tolerance to Oracle Function to check PO Balance passed 
          back to Kofax
FSC-6020: Kofax Duplicate invoice Number can be released
...
BEGIN
   ...
   INTO   l_qty_rcv_tolerance
   FROM   rcv_receiving_parameters_v
   WHERE  organization_id = p_org_id;

   dbms_output.put_line(l_qty_rcv_tolerance);

   FOR r_po IN c_po LOOP
      l_po_remain_amount := l_po_remain_amount + (r_po.amount * (NVL(l_qty_rcv_tolerance, 0) + 1));
   END LOOP;
