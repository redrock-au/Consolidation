...
/* $Header: svn://d02584/consolrepos/branches/AR.01.01/apc/1.0.0/install/sql/XXAP_SUPPLIER_CONV_PKG.pkb 2949 2017-11-13 01:09:55Z svnuser $ */
/*******************************************************************
**
** CEMLI ID: PO.00.01
**
...
--      get_fnd_user_id
--  PURPOSE
--      Queries the ID of FND user based on the user name
-- --------------------------------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------------------------------
--  PROCEDURE
--      query_related_vendor_tfm
--  PURPOSE
--      Queries a transform row from XXAP_SUPPLIER_CONV_TFM based on vendor_interface_id
-- --------------------------------------------------------------------------------------------------
PROCEDURE query_related_vendor_tfm
(
   p_vendor_interface_id   IN NUMBER,
   p_supplier_conv_tfm_rec IN OUT NOCOPY xxap_supplier_conv_tfm%ROWTYPE
) IS
   lr_tfm_rec              xxap_supplier_conv_tfm%ROWTYPE;
BEGIN
   SELECT tfm.*
   INTO   lr_tfm_rec
   ...
   AND    record_type = 'NEW';
   p_supplier_conv_tfm_rec := lr_tfm_rec; 
EXCEPTION
   WHEN NO_DATA_FOUND THEN
      p_supplier_conv_tfm_rec := NULL; 
END query_related_vendor_tfm;
...
BEGIN
   IF p_stg_rec.record_type = G_REC_TYPE_NEW AND p_stg_rec.site_name IS NULL THEN
      ...
      RETURN;
   ELSIF p_stg_rec.record_type = G_REC_TYPE_SITE_ONLY AND  p_tfm_rec.vendor_id IS NOT NULL THEN
      -- check that the site does not already exist for the mapped vendor
      query_vendor_site(p_stg_rec.site_name, p_tfm_rec.vendor_id, lr_po_vendor_sites_rec);
      IF lr_po_vendor_sites_rec.vendor_site_id IS NOT NULL THEN
         append_error(p_errors_tab, 'Site ''' || p_stg_rec.site_name || ''' already exists');
         RETURN;
      END IF;
   END IF;
   
   -- Added by Joy Pinto on 01-Sep-2017
   IF p_stg_rec.record_type = G_REC_TYPE_NEW THEN
      SELECT count(1) 
      INTO   ln_dup_ven_site_count
      FROM   xxap_supplier_conv_stg
      WHERE  run_id = p_stg_rec.run_id
      AND    supplier_number =   p_stg_rec.supplier_number
      AND    site_name       =   p_stg_rec.site_name
      AND    record_type IN ('NEW');
      
      IF ln_dup_ven_site_count > 1 THEN
         append_error(p_errors_tab, 'Source Vendor Number : '||p_stg_rec.supplier_number||' and Source Site Name : '||p_stg_rec.site_name||' combination is duplicated for NEW records');
         RETURN;      
      END IF;
   END IF;
   
   IF p_stg_rec.record_type = G_REC_TYPE_SITE_ONLY THEN
      SELECT count(1) 
      INTO   ln_dup_ven_site_count
      FROM   xxap_supplier_conv_stg
      WHERE  run_id = p_stg_rec.run_id
      AND    dtpli_supplier_num = p_stg_rec.dtpli_supplier_num
      AND    site_name          = p_stg_rec.site_name
      AND    record_type IN ('SITE_ONLY');
      
      IF ln_dup_ven_site_count > 1 THEN
         append_error(p_errors_tab, 'DTPLI Supplier Number : '||p_stg_rec.dtpli_supplier_num||' and Source Site Name : '||p_stg_rec.site_name||' combination is duplicated for SITE_ONLY records');
         RETURN;      
      END IF;
   END IF;   
...
BEGIN
  /* IF p_stg_rec.tax_registration_num IS NULL THEN
      ...
      RETURN;
   END IF;*/ -- Commented By Joy Pinto on 26-Sep-2017 to send the ABN value if its supplied and only prevent validation
...
BEGIN
   ...
   IF p_stg_rec.record_type = 'NEW' THEN
      ...
      IF lr_vendor_row.vendor_id IS NOT NULL THEN
         append_error(p_errors_tab, 'Supplier ''' || p_stg_rec.supplier_name 
            || ''' already exists, created on ' || to_char(lr_vendor_row.creation_date,'DD-MON-YYYY') || ' by user_id '|| lr_vendor_row.created_by);
         RETURN;
      END IF;
   END IF;   
...
BEGIN
   IF p_stg_rec.notification_method = 'EMAIL' THEN
      ...
      END IF;
   ELSIF p_stg_rec.notification_method = 'FAX' THEN
   ...
   ELSE 
      p_tfm_rec.site_attribute2 := p_stg_rec.notification_method; -- eft remittance type
      p_tfm_rec.site_attribute3 := NULL; -- eft email
      p_tfm_rec.site_attribute4 := NULL; -- eft fax number
   END IF;
...
(
   ...
   l_original_error_count     NUMBER := 0;
BEGIN
   -- validations and derivations
   SELECT ap_suppliers_int_s.NEXTVAL 
   INTO   p_tfm_rec.vendor_interface_id 
   FROM   DUAL;
   l_original_error_count := p_errors_tab.COUNT;
   ...
   validate_tax_registration_num(p_stg_rec, p_tfm_rec, p_errors_tab);-- Commented by Joy Pinto 24-Aug-2017
   ...
   --p_tfm_rec.one_time_flag := 'Y'; Fixed by Joy Pinto on 25-Aug-2017 
   p_tfm_rec.payment_priority := 99;
   p_tfm_rec.vendor_status := 'NEW';
   -- add the new vendor id to the cache, only if there were no validation errors
   IF p_errors_tab.COUNT = l_original_error_count THEN
...
(
   ...
   CURSOR c_sup IS
      SELECT tfm.record_type, 
             tfm.segment1, 
             tfm.vendor_site_code, 
             tfm.vendor_id, 
             tfm.vendor_site_id, 
             stg.record_id as stage_record_id,
             tfm.vendor_status,
             tfm.vendor_interface_id,
             tfm.run_id
      FROM   xxap_supplier_conv_tfm tfm,
   ...
   lr_related_conv_tfm_row    xxap_supplier_conv_tfm%ROWTYPE;
   l_dtpli_supplier_num       xxap_supplier_conv_tfm.dtpli_supplier_num%TYPE;
   l_dtpli_site_code          xxap_supplier_conv_tfm.dtpli_site_code%TYPE;
   l_debug_msg                VARCHAR2(2000);
   l_final_status             xxap_supplier_conv_tfm.status%TYPE := 'REJECTED';
   l_run_id                   NUMBER;
BEGIN
   g_debug_flag := p_debug_flag;
   FOR r_sup IN c_sup
   LOOP
      debug_msg('[' || c_sup%ROWCOUNT || '] ' || r_sup.record_type || ' ');
      l_run_id := r_sup.run_id;
      -- ----------------------------------------------------------------------------
      -- Record Type: NEW
      -- ----------------------------------------------------------------------------
      IF r_sup.record_type = G_REC_TYPE_NEW THEN
         IF r_sup.vendor_status = 'NEW' THEN
            ...
            IF lr_po_vendors_row.vendor_id IS NOT NULL THEN
               debug_msg('Initial supplier number ' || r_sup.segment1 || ' [vendor_id=' || lr_po_vendors_row.vendor_id || ']');
               l_dtpli_supplier_num := lr_po_vendors_row.vendor_id;
               -- override segment1 value with the primary key value
               UPDATE po_vendors 
               SET    segment1 = to_char(vendor_id) 
               WHERE  segment1 = r_sup.segment1
               AND    segment1 <> to_char(vendor_id);
               debug_msg('Updated po_vendors segment1 from '|| r_sup.segment1 ||' to ' || l_dtpli_supplier_num);
               ...
               IF lr_po_vendor_sites_row.vendor_site_id IS NOT NULL THEN
                  debug_msg('Site ' || r_sup.vendor_site_code || ' [vendor_site_id=' || lr_po_vendor_sites_row.vendor_site_id || ']');
                  l_dtpli_site_code := lr_po_vendor_sites_row.vendor_site_code;
                  l_final_status := 'SUCCESS';
               ELSE
                  debug_msg('Site ' || r_sup.vendor_site_code || ' not found');
                  l_dtpli_site_code := NULL; -- Added by Joy Pinto on 25-Aug-2017
               END IF;
            ELSE
               debug_msg('Initial supplier number ' || r_sup.segment1 || ' not found');
               l_dtpli_supplier_num := NULL; -- Added by Joy Pinto on 25-Aug-2017
            END IF;
         ELSE
            -- These are new secondary sites, i.e. they only have site mapping
            -- The vendor mapping is contained in the related vendor row, linked by vendor_interface_id
            query_related_vendor_tfm(r_sup.vendor_interface_id, lr_related_conv_tfm_row);
            IF lr_related_conv_tfm_row.vendor_interface_id IS NOT NULL THEN
               debug_msg('Related supplier number ' || lr_related_conv_tfm_row.segment1 || ' [vendor_id=' || lr_related_conv_tfm_row.dtpli_supplier_num || ']');
               l_dtpli_supplier_num := lr_related_conv_tfm_row.dtpli_supplier_num;
               query_vendor_site(r_sup.vendor_site_code, lr_related_conv_tfm_row.dtpli_supplier_num, lr_po_vendor_sites_row);
               IF lr_po_vendor_sites_row.vendor_site_id IS NOT NULL THEN
                  debug_msg('Site ' || r_sup.vendor_site_code || ' [vendor_site_id=' || lr_po_vendor_sites_row.vendor_site_id || ']');
                  l_dtpli_site_code := lr_po_vendor_sites_row.vendor_site_code;
                  l_final_status := 'SUCCESS';
               END IF;
            ELSE
               debug_msg('Related vendor_interface_id ' || r_sup.vendor_interface_id || ' not found');
            END IF;
         END IF;
         -- now update the TFM
         UPDATE xxap_supplier_conv_tfm
         SET    dtpli_supplier_num = l_dtpli_supplier_num,
                dtpli_site_code = l_dtpli_site_code,
                status = l_final_status
         WHERE CURRENT OF c_sup;
         debug_msg('Updated TFM (NEW) with ' || l_dtpli_supplier_num || '|' || l_dtpli_site_code || '|' || l_final_status);
      -- ----------------------------------------------------------------------------
      ...
      ELSIF r_sup.record_type = G_REC_TYPE_SITE_ONLY THEN
         IF r_sup.vendor_id IS NOT NULL THEN
            ...
            IF lr_po_vendor_sites_row.vendor_site_id IS NOT NULL THEN
               debug_msg('Site only vendor ' || r_sup.vendor_id ||' [vendor_site_id=' || lr_po_vendor_sites_row.vendor_site_id || ']');
               l_final_status := 'SUCCESS';
               ...
               SET    dtpli_site_code = lr_po_vendor_sites_row.vendor_site_code,
                      status = l_final_status
               WHERE CURRENT OF c_sup;
               debug_msg('Updated TFM (SITE_ONLY) with ' || l_dtpli_site_code || '|' || l_final_status);
            ELSE
               debug_msg('Site only vendor ' || r_sup.vendor_id ||' not found');
            END IF;
         END IF;
      -- ----------------------------------------------------------------------------
      -- Record Type: MAP
      ...
      ELSIF r_sup.record_type = G_REC_TYPE_MAP THEN
         ...
         IF r_sup.vendor_id IS NOT NULL AND r_sup.vendor_site_id IS NOT NULL THEN
            debug_msg('Map vendor [vendor_id=' || r_sup.vendor_id || '][vendor_site_id=' || r_sup.vendor_site_id || ']');
            l_final_status := 'SUCCESS';
            -- update tfm table with supplier reference and status
            UPDATE xxap_supplier_conv_tfm
            SET    status = l_final_status
            WHERE CURRENT OF c_sup;
            debug_msg('Updated TFM (MAP) with ' || l_final_status);
         END IF;
      END IF;
   END LOOP;

   BEGIN -- Added by Joy Pinto on 14-Sep-2017
      debug_msg('Starting to update vendor numbers and site codes for l_run_id = '|| l_run_id);
      FOR c_rec IN (select * from fmsmgr.xxap_supplier_conv_tfm      WHERE source_supplier_number IN
      ...
      debug_msg('Updated vendor_id and site code for record Id : '||c_rec.record_id);
   ...
   EXCEPTION
   WHEN OTHERS THEN
      debug_msg('Unexpected exception occurred while updating the transformation table - Error is : '||SQLERRM);
   END;   
...
IS
   ...
   l_val_errors_tab        t_validation_errors_type := t_validation_errors_type();
BEGIN
   /************************************/
   ...
   LOOP
      ...
      /* Mapping                         */
      /***********************************/
      r_tfm.source_supplier_number := r_stg.supplier_number;
      r_tfm.source_site_name := r_stg.site_name;
      r_tfm.record_type := r_stg.record_type;
      -- --------------------------------------------------------------------------------------------------------
      -- Record Type: NEW
...
BEGIN
   ...
   LOOP
      ...
      r_error.int_table_key_val2 := r_tfm.vendor_site_code;
      BEGIN
        IF nvl(l_vendor_name,'X') <> r_tfm.vendor_name THEN
        ...
        END IF;
         ...
         l_vendor_name := r_tfm.vendor_name;
      EXCEPTION
         WHEN OTHERS THEN
            r_error.msg_code := SQLCODE;
            r_error.error_text := SQLERRM;
...
BEGIN
   ...
   --g_conv_user_id := get_fnd_user_id('CONVERSION'); Migration issue Commeneted by Joy Pinto on 24-Aug-2017
   ...
   --debug_msg('g_conv_user_id ' || g_conv_user_id);
   debug_msg('User Running this program ' || get_fnd_user(fnd_global.user_id));
   
   IF get_fnd_user(fnd_global.user_id) <> 'CONVERSION'  THEN
      debug_msg('User Running this program is not conversion - ' || get_fnd_user(fnd_global.user_id));
...
EXCEPTION
   WHEN e_interface_error THEN
    ...
    WHEN OTHERS THEN
       debug_msg('Unexpected Error in the Vendor Conversion - SQL Error is :'||SQLERRM);
       p_retcode := 2;       
