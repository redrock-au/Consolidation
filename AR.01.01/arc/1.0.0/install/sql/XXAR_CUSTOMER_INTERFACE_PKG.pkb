...
/*$Header: svn://d02584/consolrepos/branches/AR.01.01/arc/1.0.0/install/sql/XXAR_CUSTOMER_INTERFACE_PKG.pkb 3078 2017-11-29 23:44:15Z svnuser $*/
/******************************************************************************
**
...
BEGIN
   ...
   FOR i IN 1 .. t_files.COUNT LOOP
      ...
      IF NOT file_length_check(p_source,l_file) THEN
		 ...
		 -- FSC-6023 : Corrected Error Message as per the JIRA FSC-6023	
		 fnd_file.put_line(fnd_file.log, g_debug || 'The File name ' || l_file ||' is not a correct Format');
         CONTINUE;
      END IF;
      ...
      IF NOT file_format_check(p_source,l_file) THEN
		 ...
		 -- FSC-6023 : Corrected Error Message as per the JIRA FSC-6023		 
		 fnd_file.put_line(fnd_file.log, g_debug || 'The File name' || l_file ||' is not a correct Format. Should be <Source>_CUSTOMER_<YYYYMMDDHHMMSS>.txt');
         CONTINUE;
      END IF;
