...
/* $Header: svn://d02584/consolrepos/branches/AR.02.02/arc/1.0.0/install/sql/XXAR_RECEIPT_INTERFACE_PKG.pkb 3066 2017-11-28 06:24:56Z svnuser $ */
/*******************************************************************
**
** CEMLI ID: AR.02.03
**
...
BEGIN
   IF p_stg_rec.receipt_type = 'Miscellaneous' THEN
      ...
      ELSE
         IF NVL(LENGTH(p_stg_rec.payer_name), 0) > 30 THEN -- FSC-5925
            append_error(p_errors_tab, 'Payer Name ''' || p_stg_rec.payer_name || ''' must not be more than 30 in length');
         ELSE
...
BEGIN
   ...
   LOOP
      ...
      -- FSC-5918: Remove leading zeroes
      -- validate_crn(r_stg.crn, r_tfm, l_val_errors_tab);
      validate_crn(LTRIM(TO_NUMBER(r_stg.crn)), r_tfm, l_val_errors_tab);
   ...
   LOOP
      ...
      */
      ...
      IF r_stg.payment_method = 'CHEQUE' THEN
         ...
         r_tfm.ATTRIBUTE15 := r_stg.drawer_name; -- FSC 5554 Changed by Joy Pinto on 12-Oct-2017
