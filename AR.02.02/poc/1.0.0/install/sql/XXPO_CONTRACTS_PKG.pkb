...
/* $Header: svn://d02584/consolrepos/branches/AR.02.02/poc/1.0.0/install/sql/XXPO_CONTRACTS_PKG.pkb 3066 2017-11-28 06:24:56Z svnuser $ */
/****************************************************************************
**
** CEMLI ID: PO.12.01
**
...
g_error_07             VARCHAR2(150) := 'Unable to delete file : ';
...
BEGIN
   ...
   IF f_exists THEN
      ...
      IF f_copy = 1 THEN
         --utl_file.fremove(z_src_in, p_file); FSC 3552
         f_delete := xxint_common_pkg.file_delete(p_file_name_path=>z_stg_dir || '/' || p_file);
         IF f_delete <> 1 THEN
            print_error(g_warning, g_error_07||z_stg_dir || '/' || p_file);
         END IF;         
      ELSE
...
BEGIN
   ...
   IF p_filename IS NOT NULL THEN
      ...
      f_copy := xxint_common_pkg.file_copy(p_from_path => l_in_dir || '/' || l_in_file,
                                           p_to_path => l_arc_dir || '/' || l_arc_file);
      ...
      IF f_copy = 1 THEN
         ...
         IF f_exists THEN
            ...
            -- FSC 3552 Joy Pinto 10-Oct-2017 Delete file from inbound directory
            f_delete := xxint_common_pkg.file_delete(p_file_name_path=>l_in_dir||'/'||l_in_file);  
            print_debug('Deleted the file from '||l_in_dir||' : f_delete=' || f_delete);
            f_delete := xxint_common_pkg.file_delete(p_file_name_path=>z_stg_dir||'/'||l_in_file);   
            print_debug('Deleted the file from '||z_stg_dir||' : f_delete=' || f_delete);
      ...
      ELSE
         l_message := g_error_02;
         l_message := REPLACE(l_message, '$IN_FILE', l_in_file);
         -- FSC-5985 Daniel Kwon on 17-Nov-2017 : Changed the Message from Error to Warning in case the File is not found in the inbound directory 
         print_error(g_warning, l_message);
         p_retcode := g_retcode_warning;
         /*print_error(g_error, l_message);
         p_retcode := g_retcode_error;*/
