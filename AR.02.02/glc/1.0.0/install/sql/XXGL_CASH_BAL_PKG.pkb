...
/* $Header: svn://d02584/consolrepos/branches/AR.02.02/glc/1.0.0/install/sql/XXGL_CASH_BAL_PKG.pkb 3066 2017-11-28 06:24:56Z svnuser $ */
...
BEGIN
   ...
   IF p_source = 'Receivables' THEN
      ...
      ELSIF p_sl_trx_type = 'TRADE' AND p_sl_trx_subtype = 'TRADE_UNAPP' THEN
         NULL;
   ...
   IF l_cursor_switch = 1 THEN
      LOOP
         ...
         print_debug('--------------------------------');
         print_debug(l_token_value); 
         print_debug(l_ccid);  
         print_debug(l_tax_code_id);  
         print_debug(l_amount);  
         print_debug(l_from_tax_line); 
         print_debug('--------------------------------');
         ...
         IF l_tax_code_id IS NOT NULL THEN
            l_amount := calculate_tax(TO_NUMBER(l_token_value), l_tax_code_id, l_amount, l_from_tax_line);
         ELSE
