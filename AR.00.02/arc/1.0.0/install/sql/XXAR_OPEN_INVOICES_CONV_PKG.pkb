...
/*$Header: svn://d02584/consolrepos/branches/AR.00.02/arc/1.0.0/install/sql/XXAR_OPEN_INVOICES_CONV_PKG.pkb 2447 2017-09-06 03:32:06Z jpinto $*/
/****************************************************************************
**
** CEMLI ID: AR.00.02
**
...
BEGIN
   ...
   LOOP
      ...
      -- Validate invoice Amount -- Added validation by Joy Pinto on 09-Sep-2017     
      error_token_init;      
      r_error.error_token_val1:=r_stg.INV_AMOUNT;
      BEGIN
         IF TO_NUMBER(r_stg.INV_AMOUNT) IS NOT NULL THEN
      ...
      EXCEPTION
         WHEN OTHERS THEN
            l_tfm_error := l_tfm_error + 1;
            xxint_common_pkg.get_error_message(g_error_message_48, r_error.msg_code, r_error.error_text);
            r_error.error_text := REPLACE(r_error.error_text, '$INVALID_VALUE', r_stg.INV_AMOUNT);
            r_error.error_text := REPLACE(r_error.error_text, '$COLUMN_NAME', 'INV AMOUNT');            
            raise_error(r_error);         
      END;       
...
IS
   ...
   CURSOR c_int_lines IS
      ...
      ORDER BY DSDBI_TRX_NUMBER,lpad(replace(line_number,'T',''),3,'0') asc; -- FSC-4030 Modified by Joy Pinto on 05-Sep-2017 line number ordering issue
