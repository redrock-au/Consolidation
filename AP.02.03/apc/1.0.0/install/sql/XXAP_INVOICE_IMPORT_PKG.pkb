...
/* $Header: svn://d02584/consolrepos/branches/AP.02.03/apc/1.0.0/install/sql/XXAP_INVOICE_IMPORT_PKG.pkb 1442 2017-07-04 22:35:02Z svnuser $*/
/****************************************************************************
**
** CEMLI ID: AP.03.01
**
...
BEGIN
   ...
   IF nvl(UPPER(p_notification_type),'INITIAL') = 'INITIAL' THEN
      FOR rec_run_rcpt_reminder_email IN (
                                          SELECT preparer_id, 
                                                 user_name
                                          FROM   xxpo_eligible_requestors_v
                                          WHERE  preparer_id = nvl(p_requestor_id,preparer_id)
      ...
      LOOP                
         ...
         update_item_key
                        (
                          p_item_type        => 'POCRECNO',
                          p_item_key         => lv_item_key,
                          p_preparer_id      => rec_run_rcpt_reminder_email.preparer_id
                        );     
                        
         OPEN  c_get_po_count(rec_run_rcpt_reminder_email.preparer_id);
         FETCH c_get_po_count INTO ln_po_count;
         CLOSE c_get_po_count ;
         ...
         wf_engine.setitemattrtext
                                 (
                                  ...
                                  avalue => rec_run_rcpt_reminder_email.user_name
                                 );                                                 
         wf_engine.setitemattrtext
                                 (
                                  itemtype => 'POCRECNO' ,
   ...
   ELSE -- Reminder
      FOR c_rec IN (
                    ...
                    AND    xisf.org_id = nvl(p_org_id,xisf.org_id)
                    AND NOT EXISTS (SELECT 1 from rcv_transactions x where x.attribute5 = xisf.import_id and transaction_type <> 'CORRECT') 
...
PRAGMA AUTONOMOUS_TRANSACTION;
   ...
   CURSOR c_get_kofax_invoices IS
      ...
      WHERE  nvl(invoice_imported,'N') in ('N','E')
   ...
   CURSOR c_check_po (p_po_header_id NUMBER) IS
      SELECT 'Y' 
      FROM   po_headers_all 
      WHERE  po_header_id = p_po_header_id;      

   CURSOR c_check_invoice (p_invoice_id NUMBER) IS
      SELECT 'Y' 
      FROM   ap_invoices_all 
      WHERE  po_header_id = p_invoice_id;  
      
   CURSOR c_get_requisition (p_po_header_id NUMBER) IS       
      SELECT prha.segment1,prha.requisition_header_id,prha.preparer_id
...
BEGIN
   ...
   LOOP
      ...
      FOR indx IN 1 .. c_rec.COUNT LOOP
         BEGIN
            ...
            IF c_rec(indx).po_header_id IS NULL AND c_rec(indx).invoice_id IS NULL THEN
               fnd_message.clear;
            ...
            IF c_rec(indx).po_header_id IS NOT NULL THEN
               ...
               FETCH c_get_requisition INTO c_rec(indx).requisition_id,c_rec(indx).requisition_number,c_rec(indx).preparer_id;
            ...
            FETCH c_check_vendor_id INTO lv_check_vendor;
   ...
   fnd_file.put_line(fnd_file.log,'Purging data above retention period End');
         ...
         fnd_file.new_line(fnd_file.log);
         fnd_file.put_line(fnd_file.log,'************************Summary of Invoice Load from Koafx Start****************************************************');
         fnd_file.new_line(fnd_file.log);
         IF ln_success_count >0 THEN
            fnd_file.put_line(fnd_file.log,'*********************Invoices Validated and Interfaced to custom table successfully***************************'); 
            fnd_file.new_line(fnd_file.log);
            fnd_file.put_line(fnd_file.log,'Document ID   Invoice Amount   Invoice Number   Invoice Date   Supplier Number   Supplier Site'); 
            fnd_file.new_line(fnd_file.log);
               ...
               fnd_file.put_line(fnd_file.log,
                                              ...
                                              l_success_tab(indx).supplier_site
                                );
               END LOOP;
            fnd_file.new_line(fnd_file.log);
         END IF;
         IF ln_failure_count >0 THEN
            ...
            fnd_file.new_line(fnd_file.log);
            fnd_file.put_line(fnd_file.log,'Document ID   Invoice Amount   Invoice Number   Invoice Date   Supplier Number   Supplier Site   Error Message     '); 
            fnd_file.new_line(fnd_file.log);
            ...
            fnd_file.new_line(fnd_file.log);            
         END IF;
         fnd_file.put_line(fnd_file.log,'************************Summary of Invoice Load from Koafx End****************************************************');
         fnd_file.new_line(fnd_file.LOG);
         COMMIT;
EXCEPTION 
   WHEN OTHERS THEN
...
BEGIN
   ...
   fnd_file.put_line(fnd_file.log,'Stage 1 Start - Validating and Importing Invoices from Kofax staging tables into the Custom table');
   fnd_file.new_line(fnd_file.log);
   load_invoices_from_kofax (
                               p_org_id            => p_org_id,
                               x_status            => lv_status,
   ...
   IF nvl(lv_status,'F') != 'S' THEN
      p_retcode := 1;
      fnd_file.put_line(fnd_file.log,'Stage 1 End - Invoice Load from Kofax has ended in a warning Please refer to the above log for details - Error : '||lv_err_msg);
   ELSE
      fnd_file.put_line(fnd_file.log,'Stage 1 End - Invoice Load from Kofax is successful');
   END IF;
   fnd_file.put_line(fnd_file.log,'Stage 2 Start - DEDJTR POs to Receipt Notification program');
   ...
   ELSE
         ...
         fnd_file.put_line(fnd_file.log, lv_completion_text);
         fnd_file.put_line(fnd_file.log,'Stage 2 End - Completed successfully - DEDJTR POs to Receipt Notification - request ID '||ln_request_id);
   END IF;      
   fnd_file.put_line(fnd_file.log,'Stage 3 Start - Attach URl to Historic Invoices');
   attach_url_historic
                     (
                      p_org_id       =>       p_org_id,
                      x_status       =>       lv_status
    ...
    IF nvl(lv_status,'F') = 'S' THEN
       fnd_file.new_line(fnd_file.LOG);
       fnd_file.put_line(fnd_file.log,'Stage 3 End - Historic Invoice Attachment is successful ');
    ELSE
       p_retcode := 1;
       fnd_file.put_line(fnd_file.log,'Stage 3 End - Historic Invoice Attachment is unsuccessful, please refer to the preceeding log file ');          
    END IF;
