...
/*$Header: svn://d02584/consolrepos/branches/AP.02.03/arc/1.0.0/install/sql/XXAR_CUSTOMER_CONVERSION_PKG.pkb 1806 2017-07-18 00:10:05Z svnuser $*/
/****************************************************************************
**
** CEMLI ID: AR.00.01
**
...
EXCEPTION
   WHEN OTHERS THEN
      fnd_file.put_line(fnd_file.log,'Error While Retrieving Customer Number '|| SQLERRM);
END;
...
EXCEPTION
   WHEN OTHERS THEN
      fnd_file.put_line(fnd_file.log,'Error While Retrieving Customer Site Number '|| SQLERRM);
END;
      ...
      LOOP
         FETCH main_c INTO main_r;
            EXIT WHEN main_c%NOTFOUND;
               ...
               v_dtpli_customer_num := return_customer_number(main_r.orig_system_customer_ref,main_r.customer_name);
...
IS
  ...
  BEGIN
       ...
       LOOP
          ...
          l_new_party_orig_sys_ref := get_new_party_orig_sys_ref (l_new_orig_system_ref);
            ...
            VALUES
               (l_insert_update_flag,
				        l_new_orig_system_ref,--r_site_on_tfm.orig_system_customer_ref,
				        r_site_on_tfm.orig_system_address_ref,
...
BEGIN
   ...
   FOR i IN 1 .. t_files.COUNT LOOP
      ...
      LOOP
         ...
         r_error.int_table_key_val1 := r_stg.customer_name;
         -- Transform
         r_tfm := NULL;
         ...
         BEGIN
           ...
           ELSIF  r_stg.record_action = 'SITE' THEN
             ...
             ----------------------------------------------------------------------
              ...
              /*IF NOT get_orig_system_reference(r_stg.orig_system_customer_ref) THEN
                 l_tfm_error := l_tfm_error + 1;
             ...
             /* IF get_customer(NVL(r_stg.customer_number,'X')) = 0 THEN
                 l_tfm_error := l_tfm_error + 1;
                 xxint_common_pkg.get_error_message(g_error_message_06||' Customer Number :' ||r_stg.customer_number, r_error.msg_code, l_message);
                 r_error.error_text := REPLACE(l_message, '$COL_VAL', r_error.error_token_val1);
                 raise_error(r_error);
              END IF;*/
      ...
      END;
         ...
         IF l_tfm_error > 0 THEN
            ...
            r_tfm.status := 'ERROR';
         ELSE
            -- Incoming ORIG_SYSTEM refs need to be programatically prefixed with "CONV" for both NEW and SITE record types (Rule RCI008)
            IF r_stg.record_action = 'NEW' THEN
               l_conv_prefix := 'CONV';
            ELSE
               l_conv_prefix := NULL;
            ...
            r_tfm.orig_system_customer_ref	     :=SUBSTR(l_conv_prefix||r_stg.orig_system_customer_ref,1,240);
            ...
            r_tfm.orig_system_address_ref        :=SUBSTR(l_conv_prefix||r_stg.orig_system_address_ref,1,240);
