...
/* $Header: svn://d02584/consolrepos/branches/AR.00.01/apc/1.0.0/install/sql/XXAP_INVOICE_IMPORT_PKG.pkb 2499 2017-09-08 07:20:44Z svnuser $*/
/****************************************************************************
**
** CEMLI ID: AP.03.01
**
...
BEGIN
   ...
   IF NVL(UPPER(p_notification_type), 'INITIAL') = 'INITIAL' THEN
      FOR rec_run_rcpt_reminder_email IN (SELECT preparer_id, 
                                                 user_name,
                                                 import_id
      ...
      LOOP
         ...
         fnd_file.put_line(fnd_file.log, 'wf_notification=POCRECNO-' || lv_item_key || ' import_id=' || rec_run_rcpt_reminder_email.import_id);
         fnd_file.put_line(fnd_file.log, 'preparer_id=' || rec_run_rcpt_reminder_email.preparer_id);
         ...
         update_email_sent(p_preparer_id => rec_run_rcpt_reminder_email.preparer_id,
      ...
      FOR indx IN 1 .. c_rec.COUNT LOOP
         BEGIN
            ...
            IF c_get_requestor%NOTFOUND THEN
               fnd_file.put_line(fnd_file.log, 'Unable to derive requestor name');
            ELSE
