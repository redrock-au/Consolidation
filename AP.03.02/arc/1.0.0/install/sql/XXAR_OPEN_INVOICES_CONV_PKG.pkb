...
/*$Header: svn://d02584/consolrepos/branches/AP.03.02/arc/1.0.0/install/sql/XXAR_OPEN_INVOICES_CONV_PKG.pkb 1820 2017-07-18 00:18:19Z svnuser $*/
/****************************************************************************
**
** CEMLI ID: AR.00.02
**
...
IS
   CURSOR c_trx IS
      ...
      SELECT tfm.record_id,
             ...
             to_date(stg.trx_date,'dd-mon-rrrr') trx_date,
             stg.dsdbi_crn crn,
             stg.term_name,
...
BEGIN
   ...
   LOOP
      ...
      AND    tfm.line_number = t_load(i).line_number;
 ...
 --l_batch_source_id     ra_batch_sources.batch_source_id%TYPE;
   ...
   BEGIN
         ...
         INTO   o_code_combination_id,
                ...
                o_segment6,
                o_segment7
         from fmsmgr.xxgl_coa_mapping_conv_stg
         WHERE  SOURCE_ENTITY||'-'||
                SOURCE_SOURCE||'-'||
                SOURCE_COST_CENTRE||'-'||
                SOURCE_ACCOUNT||'-'||
                SOURCE_PROJECT = p_old_charg_code;
   EXCEPTION
       WHEN NO_DATA_FOUND THEN
   ...
   /*PROCEDURE get_tax_account (p_org_id NUMBER,
                              p_sob_id NUMBER,
                              p_tax_code IN ar_vat_tax_all_b.tax_code%TYPE,
                              p_tax_account_id OUT ar_vat_tax_all_b.tax_account_id%TYPE ,
                              p_tax_segment1 OUT gl_code_combinations.SEGMENT1%TYPE,
                              p_tax_segment2 OUT gl_code_combinations.SEGMENT2%TYPE,
                              p_tax_segment3 OUT gl_code_combinations.SEGMENT3%TYPE,
                              p_tax_segment4 OUT gl_code_combinations.SEGMENT4%TYPE,
                              p_tax_segment5 OUT gl_code_combinations.SEGMENT5%TYPE,
                              p_tax_segment6 OUT gl_code_combinations.SEGMENT6%TYPE,
                              p_tax_segment7 OUT gl_code_combinations.SEGMENT7%TYPE)
   IS
      CURSOR tax_account_c IS
        ...
        AND set_of_books_id = p_sob_id
        AND org_id = p_org_id
        AND tax.tax_account_id = gcc.code_combination_id;
   BEGIN
      FOR tax_account_r IN tax_account_c LOOP
         p_tax_account_id := tax_account_r.tax_account_id;
         p_tax_segment1 := tax_account_r.segment1;
         p_tax_segment2 := tax_account_r.segment2;
         p_tax_segment3 := tax_account_r.segment3;
         p_tax_segment4 := tax_account_r.segment4;
         p_tax_segment5 := tax_account_r.segment5;
         p_tax_segment6 := tax_account_r.segment6;
         p_tax_segment7 := tax_account_r.segment7;
      END LOOP;
   END get_tax_account;*/
...
BEGIN
   ...
   LOOP
      ...
      r_tfm.INVOICE_EMAIL:= SUBSTR(r_stg.INVOICE_EMAIL,150);                          
      ...
      -- Check whether the AR Invoice is Partially paid adjustment or not by Comparing Open Amount and Sum of Line Amount --
      
      get_sum_amounts (r_stg.DSDBI_TRX_NUMBER,                        
                       l_open_amount,
                       l_actual_amount);
      
      
      IF l_open_amount <> l_actual_amount THEN
      -- The Line Amounts need to be reduced pro-rata to ensure that Amount Outstanding = sum of Line Amounts. 
      -- In this way, the invoice will effectively be migrated for only the outstanding amount.
         
            v_new_line_amount   := round(l_open_amount*(r_stg.LINE_AMOUNT/l_actual_amount),2);
            --r_tfm.LINE_AMOUNT   := v_new_line_amount;
            v_new_line_quantity := round(v_new_line_amount,2)/r_stg.UNIT_SELLING_PRICE;
            r_tfm.LINE_QUANTITY := v_new_line_quantity; 
            
            IF r_tfm.line_type = 'LINE' THEN
               r_tfm.LINE_AMOUNT :=  v_new_line_quantity * r_stg.UNIT_SELLING_PRICE;
            ELSIF r_tfm.line_type = 'TAX' THEN
               r_tfm.LINE_AMOUNT := get_new_tax_amount(g_org_id,
                                                       g_sob_id,
                                                       r_stg.DSDBI_TRX_NUMBER,
                                                       r_stg.line_number,
                                                       r_stg.tax_code);
            END IF;
            
            PRINT_DEBUG('r_tfm.LINE_QUANTITY :'||r_tfm.LINE_QUANTITY);
 
            IF r_tfm.DIST_LINE_TYPE = 'REC' THEN
               r_tfm.DIST_AMOUNT:=  l_open_amount;
            ELSE 
               r_tfm.DIST_AMOUNT:=  r_tfm.LINE_AMOUNT;
            END IF;
     ...
     /* -- Grouping Rules
      ...
      r_tfm.interface_line_attribute2 := r_stg.DSDBI_TRX_NUMBER;
      r_tfm.interface_line_attribute3 := r_stg.LINE_NUMBER;*/
     ...
     END IF;
      ...
      ELSE
         IF LENGTH(TRIM(r_stg.DSDBI_TRX_NUMBER)) > 20 THEN
            ...
            r_error.error_text := REPLACE(r_error.error_text, '$DSDBI_TRX_NUMBER', r_stg.DSDBI_TRX_NUMBER);
            raise_error(r_error);
         ELSE
     ...
     r_error.error_token_val1 := r_stg.charge_code;
      ...
      ELSE
         ...
         --  ELSIF r_stg.line_type = 'TAX' and r_stg.tax_code IS NOT NULL THEN
          /*    get_tax_account (g_org_id,
                              g_sob_id,
                              r_stg.tax_code,
                              r_tfm.code_combination_id,
                              r_tfm.segment1,
       ...
       --    END IF;
           ...
           /*r_tfm.code_combination_id := fnd_flex_ext.get_ccid(application_short_name => 'SQLGL',
                                                            ...
                                                            concatenated_segments => r_tfm.segment1||'-'||
                                                                                     r_tfm.segment2||'-'||
                                                                                     r_tfm.segment3||'-'||
                                                                                     r_tfm.segment4||'-'||
                                                                                     r_tfm.segment5||'-'||
...
IS
  ...
  SUBTYPE int_dists_rec_type IS c_int_dists%ROWTYPE;
   ...
   FUNCTION get_mapped_customer_id (p_customer_number VARCHAR2)
   RETURN NUMBER IS
      o_new_customer_id RA_CUSTOMERS.CUSTOMER_ID%TYPE;
   BEGIN
   ...
   EXCEPTION
      WHEN NO_DATA_FOUND THEN
         r_error.run_id := p_run_id;
         r_error.int_table_key_val1 := p_customer_number;
         xxint_common_pkg.get_error_message(g_error_message_40, r_error.msg_code, r_error.error_text);
         r_error.error_text := REPLACE(r_error.error_text, '$CUSTOMER_NUM', p_customer_number);
         raise_error(r_error);
      WHEN OTHERS THEN
         r_error.run_id := p_run_id;
         r_error.int_table_key_val1 := p_customer_number;
         xxint_common_pkg.get_error_message(SQLERRM, r_error.msg_code, r_error.error_text);
         raise_error(r_error);         
   ...
   RETURN VARCHAR2 IS
      o_orig_sys_bill_address_ref  hz_party_sites.party_site_id%TYPE;
   BEGIN
      /*SELECT distinct dtpli_customer_site_num
      INTO o_new_cust_site_number
      ...
      SELECT hcas.ORIG_SYSTEM_REFERENCE--hps.party_site_id
   ...
   EXCEPTION
      WHEN NO_DATA_FOUND THEN
         r_error.run_id := p_run_id;
         r_error.int_table_key_val1 := p_customer_site_number;
         xxint_common_pkg.get_error_message(g_error_message_41, r_error.msg_code, r_error.error_text);
         r_error.error_text := REPLACE(r_error.error_text, '$CUSTOMER_SITE_NUM', p_customer_site_number);
         raise_error(r_error);   
      WHEN OTHERS THEN
         r_error.run_id := p_run_id;
         r_error.int_table_key_val1 := p_customer_site_number;
...
BEGIN
   ...
   IF g_int_mode = 'VALIDATE_TRANSFER' THEN
      ...
      LOOP
         ...
         v_orig_sys_bill_customer_ref:= get_mapped_customer_id(t_int_lines.dsdbi_customer_number);
         v_orig_sys_sold_customer_ref:= get_mapped_customer_id(t_int_lines.dsdbi_customer_number); 
         
         -- ARLRUL09 
         
         v_orig_sys_bill_address_ref := get_mapped_bill_address_ref(t_int_lines.dsdbi_customer_number ,t_int_lines.dsdbi_customer_site_number);
         ...
         VALUES (
                 ...
                 g_org_id,--101 ,                       
                 v_orig_sys_bill_address_ref,--t_int_lines.dsdbi_customer_site_number , 
                 v_orig_sys_bill_customer_ref,
                 v_orig_sys_sold_customer_ref,
