...
/* $Header: svn://d02584/consolrepos/branches/AP.03.02/apc/1.0.0/install/sql/XXAP_INVOICE_INTERFACE_PKG.pkb 1071 2017-06-21 05:16:54Z svnuser $ */
/*******************************************************************
**
** CEMLI ID: AP.02.01
**
...
BEGIN
   IF p_stg_rec.invoice_date IS NOT NULL THEN
      ...
      IF NOT g_period_cache.EXISTS(p_tfm_rec.invoice_date) THEN
         query_period_statuses(p_tfm_rec.invoice_date, 'MONTH', 200, lr_gl_period_statuses_rec);
         IF ( lr_gl_period_statuses_rec.period_name IS NULL OR 
              lr_gl_period_statuses_rec.closing_status != 'O' ) 
         THEN
            append_error(p_errors_tab, 'Invoice Date ''' || p_stg_rec.invoice_date 
               || ''' is not in an open GL period');
            RETURN;
         ELSE
...
IS
   ...
   l_ap_tax_codes_rec      ap_tax_codes%ROWTYPE;
...
BEGIN
   ...
   LOOP
      ...
      r_tfm.GL_DATE := r_tfm.invoice_date;
      r_tfm.ORG_ID := g_org_id;
      r_tfm.ACCTS_PAY_CODE_COMBINATION_ID := r_tfm.dist_code_combination_id;
      ...
      r_tfm.ACCOUNTING_DATE := r_tfm.invoice_date;
...
BEGIN
   ...
   LOOP
      ...
      /* Copy APXIIMPT files           */
      /*********************************/
      copy_conc_request_file(l_apxiimpt_req_id, 'OUTPUT', l_outbound_directory, replace(l_ack_file_name, l_req_id, l_req_id || '_out'));
      copy_conc_request_file(l_apxiimpt_req_id, 'LOG', l_outbound_directory, replace(l_ack_file_name, l_req_id, l_req_id || '_log'));
