...
g_int_name                  dot_int_interfaces.int_name%TYPE := 'DEDJTR GL Account Balance Conversion';
...
-- For Test
--g_user_je_category_name     gl_interface.user_je_category_name%TYPE := 'Other';
--g_user_je_source_name     gl_interface.user_je_source_name%TYPE := 'Manual';


-- Defaulting Rules --
/*g_file            VARCHAR2(150)  :=  'CODE_COMB_COA_MAPPINGS.csv';
g_ctl             VARCHAR2(150)   := '$GLC_TOP/bin/XXGLCOABAL.ctl';*/
...
g_org_id               NUMBER;
g_accounting_date      VARCHAR2(30);
g_balance_or_net_flag  VARCHAR2(1);
...
BEGIN
   ...
   p_inbound_directory := xxint_common_pkg.interface_path(p_application => g_appl_short_name,
                                                          p_source => p_source,
                                                          p_message => p_message);
DBMS_OUTPUT.PUT_LINE ('get_files-001 '||p_inbound_directory);
   IF p_message IS NOT NULL THEN
      RETURN FALSE;
   END IF;
DBMS_OUTPUT.PUT_LINE ('get_files-002 '||p_file_string);   
DBMS_OUTPUT.PUT_LINE ('get_files-002 '||p_source);   
   ...
   l_file_req_id := fnd_request.submit_request(application => 'FNDC',
                                               ...
                                               argument4   => g_appl_id);
   COMMIT;
DBMS_OUTPUT.PUT_LINE ('get_files-003 '||l_file_req_id);   
...
DBMS_OUTPUT.PUT_LINE ('get_files-004 '||srs_dev_phase);   

   ELSE

DBMS_OUTPUT.PUT_LINE ('get_files-005 '||srs_dev_phase);   
...
DBMS_OUTPUT.PUT_LINE ('get_files-005 ');   

   -- Interface control record
   r_request.application_id := g_appl_id;

DBMS_OUTPUT.PUT_LINE ('get_files-006 ');      

   r_request.interface_request_id := g_interface_req_id;

DBMS_OUTPUT.PUT_LINE ('get_files-007 ');      


   r_request.sub_request_id := l_file_req_id;

DBMS_OUTPUT.PUT_LINE ('get_files-008 '||g_interface_req_id);      
...
DBMS_OUTPUT.PUT_LINE ('get_files-009 ');      
...
BEGIN
  null;
   ...
   xxint_common_pkg.g_object_type := 'JOURNALS'; -- Added as interface directories are repointed to NFS
   -- Global Variables
   g_debug_flag := 'Y'; --NVL(p_debug_flag, 'N');
   ...
   fnd_file.put_line(fnd_file.log,'001');

   l_ctl :=p_control_file;
   l_file := p_file_name;
   
   /*IF p_file_name LIKE '%_B.csv' THEN
      g_balance_or_net_flag := 'B';
   ELSIF p_file_name LIKE '%_P.csv' THEN
      g_balance_or_net_flag := 'P'; 
   END IF;*/
   ...
   IF NOT get_files(p_source => l_source,
                    ...
                    p_message => l_message)
   THEN
      DBMS_OUTPUT.PUT_LINE('008-1'||l_source);
      DBMS_OUTPUT.PUT_LINE('008-1'||l_file);
      DBMS_OUTPUT.PUT_LINE('008-1'||l_inbound_directory);
      DBMS_OUTPUT.PUT_LINE('008-1'||t_files.COUNT);
      DBMS_OUTPUT.PUT_LINE('008-1'||l_message);
      RAISE interface_error;
   END IF;
   ...
   FOR i IN 1 .. t_files.COUNT LOOP
    ...
    IF p_draft = 'Y' THEN
        print_debug('Draft Mode is '||p_draft);
        print_debug('Progam finished Transformation');
        print_debug('Manual Journal Import is required to create a journal once Data verification completes');
        
        draft_mode_print_output(l_run_id,
                   l_run_transform_id,
                   l_request_id,
                   l_source,
                   l_file,
...
IS
   CURSOR c_stg IS
      SELECT stg.record_id,
             ...
             'Period : '||stg.period_name||'-DSDBI COA:'||stg.old_segment1||'.'||stg.old_segment2||'.'||stg.old_segment3||'.'||stg.old_segment4||'.'||stg.old_segment5 reference10
   ...
   BEGIN
      ...
      SET    status = 'ERROR'
      WHERE  NVL(status, 'NEW') <> 'ERROR'
      AND    run_id = p_run_id;
   END set_to_error;
   
   PROCEDURE get_mapped_breakup_segments (p_old_combination IN VARCHAR2,
                                o_new_segment1 OUT VARCHAR2,
                                o_new_segment2 OUT VARCHAR2,
                                o_new_segment3 OUT VARCHAR2,
                                o_new_segment4 OUT VARCHAR2,
                                o_new_segment5 OUT VARCHAR2,
                                o_new_segment6 OUT VARCHAR2,
                                o_new_segment7 OUT VARCHAR2) IS
                                
        x_ccid VARCHAR2(500);
        x_error_msg VARCHAR2(500);

        v_new_mapped_coa VARCHAR2(500);
        v_a VARCHAR2(500);
        --l_old_segs                NUMBER;
        l_new_segs                NUMBER;
        --old_t_segments            fnd_flex_ext.SegmentArray;
        new_t_segments            fnd_flex_ext.SegmentArray;
        
        
   BEGIN
       
       v_new_mapped_coa := xxgl_import_coa_mapping_pkg.get_mapped_code_combination(p_old_combination,x_ccid,x_error_msg);
       ...
       IF x_error_msg IS NOT NULL THEN
          ...
          r_error.error_text := REPLACE(r_error.error_text||x_error_msg, '$OLD_COMBINATION',p_old_combination );
          raise_error(r_error);
          print_debug(g_error_message_10);
       END IF;
       
       
       l_new_segs := fnd_flex_ext.breakup_segments(concatenated_segs => v_new_mapped_coa,
                                                    delimiter         => '-',
                                                    segments          => new_t_segments);

       o_new_segment1 := new_t_segments(1);
       o_new_segment2 := new_t_segments(2);
       o_new_segment3 := new_t_segments(3);
       o_new_segment4 := new_t_segments(4);
       o_new_segment5 := new_t_segments(5);
       o_new_segment6 := new_t_segments(6);
       o_new_segment7 := new_t_segments(7);
      
   EXCEPTION
      WHEN OTHERS THEN
         l_tfm_error := l_tfm_error + 1;
         xxint_common_pkg.get_error_message(g_error_message_10, r_error.msg_code, r_error.error_text);
         r_error.error_text := REPLACE(r_error.error_text||l_error_message, '$OLD_COMBINATION',p_old_combination );
         raise_error(r_error);
         print_debug(g_error_message_10);
   END get_mapped_breakup_segments;
   
   PROCEDURE check_unbalanced_segment (p_run_id NUMBER) 
   IS
...
BEGIN
   ...
   LOOP
    ...
    print_debug('004');
      ...
      --r_tfm.OLD_CODE_COMBINATION  := SUBSTR(r_stg.DISTRIBUTION , 1, 24);
    ...
    print_debug('006');
      ...
      /*get_mapped_breakup_segments (r_stg.DISTRIBUTION,
                                l_target_seg1,
                                l_target_seg2,
                                l_target_seg3,
                                l_target_seg4,
      ...
      -- Batch, Header, Line Descriptions
...
IS
   ...
   SUBTYPE int_journal_rec_type IS c_int_journal%ROWTYPE;
   TYPE    int_journal_tab_type IS TABLE OF int_journal_rec_type INDEX BY binary_integer;

   t_int_journal         int_journal_tab_type;
   l_record_count      NUMBER := 0;
   l_success_count     NUMBER := 0;
   l_error_count       NUMBER := 0;
   l_code              VARCHAR2(15);
...
BEGIN
   ...
   IF g_int_mode = 'VALIDATE_TRANSFER' THEN
      ...
      -- Interface Table: Header and Lines
         ...
         g_accounting_date := LAST_DAY(TO_DATE('JAN-17','MON-RRRR'));
...
IS
   CURSOR c_results IS
      SELECT  STATUS,
        PERIOD_NAME,
        ACTUAL_FLAG,
        OLD_SEGMENT1||'-'||OLD_SEGMENT2||'-'||OLD_SEGMENT3||'-'||OLD_SEGMENT4||'-'||OLD_SEGMENT5 OLD_CODE_COMBINATION,
        OLD_CODE_COMBINATION_ID,
        NEW_CODE_COMBINATION_ID,
        NEW_SEGMENT1||'-'||NEW_SEGMENT2||'-'||NEW_SEGMENT3||'-'||NEW_SEGMENT4||'-'||NEW_SEGMENT5||'-'||NEW_SEGMENT6||'-'||NEW_SEGMENT7 NEW_CODE_COMBINATION,
        CLOSE_BALANCE_DR,
        CLOSE_BALANCE_CR,
        CURRENCY_CODE,
        RECORD_ID
