...
/* $Header: svn://d02584/consolrepos/branches/AP.01.01/glc/1.0.0/install/sql/XXGL_PAYROLL_PKG.pkb 2674 2017-10-05 01:02:16Z svnuser $ */
/****************************************************************************
**
** CEMLI ID: GL.02.01
**
...
(
  ...
  CURSOR c_detail IS
   ...
   l_detail_total NUMBER := 0;   
   l_wip_category               xxgl_payroll_detail_stg.record_category%TYPE;
   l_format_mask              VARCHAR2(30);
BEGIN
   ...
   FOR r_detail IN c_detail LOOP
       fnd_file.put_line(fnd_file.output, 
                         ...
                         ||rpad('From :'|| r_detail.STG_CC,50,' ') 
                         ||rpad('To :'|| r_detail.TFM_CC,36,' ')
                         ||lpad(to_char(r_detail.amount, l_format_mask), 31, ' ')
                         );
       l_detail_total := l_detail_total +  r_detail.amount;                                                              
   END LOOP;
   
   fnd_file.put_line(fnd_file.output, ' ');
   fnd_file.put_line(fnd_file.output, LPAD('--------------------------',152,' '));
   fnd_file.put_line(fnd_file.output,  rpad('Total:', 90, ' ')   || lpad(to_char(l_detail_total, l_format_mask), 59, ' '));
   fnd_file.put_line(fnd_file.output, ' ');
   fnd_file.put_line(fnd_file.output,  '                                                           **      End of Report    **            ');
