...
/* $Header: svn://d02584/consolrepos/branches/AP.03.01/arc/1.0.0/install/sql/XXAR_PRINT_INVOICES_PKG.pkb 3181 2017-12-12 07:17:14Z svnuser $ */
/******************************************************************************
**
**
...
IS
   ...
   CURSOR c_prt_line (p_this_request NUMBER) IS
      SELECT customer_trx_id, -- FSC-6016
             'XXARINVPSREM_' || p.sub_request_id || '_1.PDF' file_name  --Rao (CEMLI AR.01.01)
      FROM   dot_ar_print_invoices p
   ...
   CURSOR c_email (p_this_request NUMBER) IS
    ...
    --RRAM transactions exempted from email
      AND    NOT EXISTS (SELECT 1
                         FROM  ra_cust_trx_types_all rc, 
   ...
   IF p_trx_number_low IS NOT NULL THEN
      BEGIN
         ...
         */
         ...
         -- 18/09/2017 FSC-4064, FSC-3587
         -- Use latest AR Transaction ID in case duplicates exist
         SELECT customer_trx_id
         INTO   ln_trx_id_low
         FROM   ra_customer_trx_all a
         WHERE  a.trx_number = p_trx_number_low
         ...
         IF sw_rec = 1 THEN
            ...
            */
            ...
            ln_par_req_id := fnd_request.submit_request(application => 'ARC',
                                                        ...
                                                        argument13  => 'Y',
                                                        argument14  => NULL,
                                                        argument15  => r_inv.customer_trx_id);
            ...
            IF p_printer_name IS NOT NULL THEN
               ...
               */
               ...
               IF p_print_command_line = 'N' THEN
                  ...
                  -- S.Ryan 26-Feb-2016  
                  -- Rao 14/Jun/2017 - used new concurrent program name (CEMLI AR.01.01)
                  fnd_file.put_line(fnd_file.log, 'Concurrent Copies set to 1 (' || p_printer_name ||
                                    ') on submission of XXARINVPSREM'); 
               ELSE
                  -- Setting copies to zero to prevent spooling of PDF to printer
   ...
   */
   ...
   IF p_print_command_line = 'Y' AND
      ...
      FOR r_prt IN c_prt_line (ln_this_request) LOOP
         ...
         SELECT COUNT(1)
         INTO   ln_mail_count
         FROM   xxar_print_invoices_v api
         WHERE  api.customer_trx_id = r_prt.customer_trx_id
         AND    NVL(api.email_address, api.feedersys_email) IS NOT NULL
         AND    NOT EXISTS (SELECT 1
                            FROM   ra_cust_trx_types_all rc, 
                                   ra_customer_trx_all rct
                            WHERE  rc.cust_trx_type_id = rct.cust_trx_type_id
         ...
         IF ln_mail_count > 0 AND  p_email_function = 'Y' THEN  
            -- FSC-6016
            NULL;
         ELSE
            b_print := fnd_request.set_print_options(copies => 0);
            ...
            COMMIT;
            fnd_file.put_line(fnd_file.log, 'DEDJTR Print Command Line called. Request ID :' || ln_sub_req_id);
            fnd_file.put_line(fnd_file.log, '**Print Complete');
         END IF;
      END LOOP;
   END IF;
...
BEGIN
   ...
   LOOP
      ...
      BEGIN
         ...
         IF r_out.feedersys_email IS NOT NULL AND
            ((lt_email.COUNT=1 AND r_out.feedersys_email <> lt_email(1)) OR (lt_email.count > 1)) THEN
