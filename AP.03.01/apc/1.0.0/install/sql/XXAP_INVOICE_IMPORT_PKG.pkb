...
/* $Header: svn://d02584/consolrepos/branches/AP.03.01/apc/1.0.0/install/sql/XXAP_INVOICE_IMPORT_PKG.pkb 3194 2017-12-19 04:26:35Z dart $*/
/****************************************************************************
**
** CEMLI ID: AP.03.01
**
...
IS
   CURSOR c_get_reminder_requestors_dtl IS
      ...
      AND    (SELECT COUNT(1)
              ...
              WHERE  a.invoice_id = xisf.invoice_id) = 0 -- Joy Pinto FSC-4197
...
-------------------------------------------------------
-- Procedure
--     CREATE_WF_REMINDER_EXT
-- Purpose
--     Run this procedure to build notification message
--     body from static data generated from the time
--     the concurrent program is submitted. Applied to
--     fix defect FSC-5956.
--     arellanod 2017-11-20
-------------------------------------------------------

PROCEDURE create_wf_reminder_ext
(
   document_id   IN VARCHAR2,
   display_type  IN VARCHAR2,
   document      IN OUT nocopy VARCHAR2,
...
IS
   CURSOR c_get_reminder_requestors_dtl IS
      SELECT pwf.invoice_num,
             pwf.po_number,
             pwf.import_date,
             pwf.attachment_url
      FROM   xxap_po_receipt_static_wf pwf
      WHERE  pwf.wf_item_key = document_id;

   CURSOR c_get_invoice_count IS
      SELECT COUNT(DISTINCT pwf.invoice_num)
      FROM   xxap_po_receipt_static_wf pwf
      WHERE  pwf.wf_item_key = document_id;
...
BEGIN
   ...
   FOR c_rec IN  c_get_reminder_requestors_dtl LOOP
      BEGIN
         lv_body := lv_body || '<tr>
                    <td>' || '<a href="' || c_rec.attachment_url || '" target="_blank">' || c_rec.invoice_num || '</a>' || '</td>
                    <td>' || c_rec.po_number|| '</td>
                    <td>' || c_rec.import_date|| '</td>
                    </tr>';
      END;
...
END create_wf_reminder_ext;

PROCEDURE create_static_lines
(
...
IS
   l_preparer_id    NUMBER := p_preparer_id;
   l_days_old       NUMBER := p_days_old;
   l_item_key       VARCHAR2(60) := p_item_key;

   PRAGMA           autonomous_transaction;
BEGIN
   -- Purge old data
   DELETE FROM xxap_po_receipt_static_wf
   WHERE  creation_date < (TRUNC(SYSDATE) - z_default_retention);

   -- Create temporary invoice line information
   INSERT INTO xxap_po_receipt_static_wf
   SELECT invoice_num,
          po_number ,
          creation_date,
          ...
          fnd_global.user_id,
          SYSDATE
   FROM   xxap_inv_scanned_file xs
   WHERE  xs.preparer_id = p_preparer_id
   AND    xs.email_sent_date IS NOT NULL
   AND    TRUNC(SYSDATE) - TRUNC(xs.email_sent_date) >= p_days_old
   AND    xs.po_header_id IS NOT NULL
   AND    NVL(xs.active_flag, 'Y') = 'Y'
   AND    xs.org_id = p_org_id
   AND    (SELECT COUNT(1)
           FROM   ap_invoices_all a
           WHERE  a.invoice_id = xs.invoice_id) = 0
   AND    (xs.active_flag IS NULL OR xs.active_flag = 'Y')
   AND    NOT EXISTS (SELECT 1
                      FROM   (SELECT h.receipt_num,
                                     t.attribute5 import_id,
                                     h.vendor_id,
                              ...
                              GROUP  BY h.receipt_num,
                                        ...
                                        h.vendor_id
                              HAVING (NVL(SUM(t.quantity), 0) + NVL(SUM(t.amount), 0)) <> 0) a
                      WHERE   a.import_id = xs.import_id
                      AND     a.vendor_id = xs.vendor_internal_id);

   COMMIT;

END create_static_lines;
...
BEGIN
   ...
   ELSE
      ...
      LOOP
         ...
         -- arellanod 2017/11/20 FSC-5956
         -- create static lines (temp data)
         create_static_lines(c_rec.preparer_id, p_org_id, p_days_old, lv_item_key);
         ...
         IF lv_send_email_flag = 'Y' THEN
          ...
          --l_document_id := 'PLSQL:XXAP_INVOICE_IMPORT_PKG.CREATE_WF_REMINDER_DOC/' || c_rec.preparer_id || '~' || p_days_old;
            ...
            l_document_id := 'PLSQL:XXAP_INVOICE_IMPORT_PKG.CREATE_WF_REMINDER_EXT/' || lv_item_key;
...
--     ATTACH_URL_HISTORIC
-- Purpose
--     Create as attachment - URL of invoice image
-------------------------------------------------------
...
BEGIN
   ...
   FOR c_rec IN c_get_unattached_invoices LOOP
      ...
      BEGIN
         ...
         AND    import_id = c_rec.import_id;

         COMMIT;

         fnd_file.put_line(fnd_file.log, 'Invoice Number: ' || c_rec.invoice_num || ' Attachment: Y');

      EXCEPTION
         WHEN others THEN
            fnd_file.put_line(fnd_file.log, 'Invoice Number: ' || c_rec.invoice_num || ' Attachment: N (Error: ' || SQLERRM || ')'); 
...
EXCEPTION
   WHEN others THEN
      ROLLBACK;
      x_status := 'F';
      fnd_file.put_line(fnd_file.log, 'Unexpected error during execution of procedure (attach_url_historic: ' || SQLERRM || ')');
...
IS
 ...
 --TYPE inv_stg_tab IS TABLE OF xxap_inv_scanned_file%ROWTYPE;
   ...
   CURSOR c_get_kofax_invoices IS
      ...
      WHERE  NVL(invoice_imported, 'N') = 'N' -- FSC-4197 Do not reprocess 'E' errored records
      ...
      FOR indx IN 1 .. c_rec.COUNT LOOP
         BEGIN
            ...
            IF c_rec(indx).po_header_id IS NOT NULL THEN
               ...
               /* FSC-3613 Joy Pinto 22-Aug-2017
         ...
         EXCEPTION
            WHEN others THEN
               fnd_file.put_line(fnd_file.log, 'Unexpected error while validating document id: ' || c_rec(indx).import_id || ' (' || SQLERRM || ')');
               ROLLBACK;
         END ;
      END LOOP;
   END LOOP;

   fnd_file.put_line(fnd_file.log, 'STAGE 1: Success count = ' || ln_success_count);
   fnd_file.put_line(fnd_file.log, 'STAGE 1: Failure count = ' || ln_failure_count);

   /*
   -----------------------------------------------------------------
   -- Purge the records which are older than retention days       --
   -- Added by Joy Pinto on 20-Jun-2016 as per Email from Pankaj  --
   -- for deleting the records from XXAP_KOFAX_INV_STG            --
   -----------------------------------------------------------------
   */
   fnd_file.put_line(fnd_file.log, 'STAGE 1: Purge old staging data');
   purge_interface_data('XXAP_KOFAX_INV_STG', NVL(fnd_profile.value('XXINT_DEFAULT_RETENTION'), 90));

   fnd_file.put_line(fnd_file.output, '');
   fnd_file.put_line(fnd_file.output, 'Document ID   Invoice Amount   Invoice Number   Invoice Date   Supplier Name             Supplier Number   Supplier Site   PO Number   Requestor   ');
   fnd_file.put_line(fnd_file.output, '');

   IF ln_success_count > 0 THEN
      FOR indx in l_success_tab.FIRST .. l_success_tab.LAST LOOP
         /* FSC-5999 arellanod 2017/12/11
   ...
   fnd_file.put_line(fnd_file.output, 'Success Count: ' || ln_success_count);
   fnd_file.put_line(fnd_file.output, '');
   fnd_file.put_line(fnd_file.output, 'Document ID   Invoice Amount   Invoice Number   Invoice Date   Supplier Name             Supplier Number   Supplier Site   PO Number   Requestor      Error Message     ');
   fnd_file.put_line(fnd_file.output, '');
   ...
   fnd_file.put_line(fnd_file.output, 'Failure Count: ' || ln_failure_count);
   fnd_file.put_line(fnd_file.output, '');
...
EXCEPTION
   WHEN others THEN
      fnd_file.put_line(fnd_file.log, 'STAGE 1: Unexpected error during execution of procedure (load_invoices_from_kofax: ' || SQLERRM || ')');
      x_status :='E';
      x_err_msg := SQLERRM;
      ROLLBACK;
...
BEGIN
   ...
   fnd_file.put_line(fnd_file.output, '');
   fnd_file.put_line(fnd_file.output, lv_program_name);
   fnd_file.put_line(fnd_file.output, 'Run Date: ' || TO_CHAR(SYSDATE, 'YYYY/MM/DD HH24:MI:SS'));
   fnd_file.put_line(fnd_file.output, 'User:     ' || lv_user_name);
   fnd_file.put_line(fnd_file.output, '');
   ...
   fnd_file.put_line(fnd_file.log, 'STAGE 1: Validate invoices from staging');
   ...
   IF NVL(lv_status, 'F') = 'S' THEN
      fnd_file.put_line(fnd_file.log, 'STAGE 1: Comleted');
   ELSE
      fnd_file.put_line(fnd_file.log, 'STAGE 1: Completed with warning status ' || lv_err_msg);
      ln_warning := ln_warning + 1;
   END IF;
   ...
   fnd_file.put_line(fnd_file.log, 'STAGE 2: Dispatch workflow notification (Request ID: ' || ln_request_id || ')');
   ...
   IF NOT (srs_dev_phase = 'COMPLETE' AND
      ...
      fnd_file.put_line(fnd_file.log, 'STAGE 2: Completed with error status');
      ln_warning := ln_warning + 1;
   ELSE
      fnd_file.put_line(fnd_file.log, 'STAGE 2: Completed');
   END IF;

   fnd_file.put_line(fnd_file.log, 'STAGE 3: Process historical invoice attachment');

   attach_url_historic(p_org_id => p_org_id,
                       x_status => lv_status);

   IF nvl(lv_status, 'F') = 'S' THEN
      fnd_file.put_line(fnd_file.log, 'STAGE 3: Completed');
   ELSE
      fnd_file.put_line(fnd_file.log, 'STAGE 3: Completed with error status');
      ln_warning := ln_warning + 1;
   END IF;
   ...
   fnd_file.put_line(fnd_file.output, '');
   fnd_file.put_line(fnd_file.output, LPAD('*** End of Process Run ***', 95, ' '));

EXCEPTION
   WHEN others THEN
      fnd_file.put_line(fnd_file.log, 'Unexpected error during execution of program (import_invoices: ' || SQLERRM || ')');
      p_retcode := 2;
...
--     DEACTIVATE_INVOICE
-- Purpose
--     This program is registered to run as a concurrent job
--     (1) Updates XXAP_INV_SCANNED_FILE active_flag to 'N' and
--         XXAP_KOFAX_INV_STG invoice_imported to 'D'
--     (2) Table XXAP_INV_SCANNED_FILE is polled by Kofax
--         for deactivated records. It deletes all corresponding 
--         record in Kofax server.
-------------------------------------------------------------------
...
IS
   ...
   lv_conc_prog_name     fnd_concurrent_programs_tl.user_concurrent_program_name%TYPE;
   lv_user_name          fnd_user.user_name%TYPE;
BEGIN
   -- FSC-6093 arellanod 2017/12/12
   -- Include information regarding deactivated
   -- Kofax invoice
   ln_conc_req_id := fnd_global.conc_request_id;
   ln_conc_prog_id := fnd_global.conc_program_id;
   lv_user_name := fnd_global.user_name;

   IF ln_conc_prog_id IS NOT NULL THEN
      SELECT p.user_concurrent_program_name
      INTO   lv_conc_prog_name
      FROM   fnd_concurrent_programs_tl p
      WHERE  p.concurrent_program_id = ln_conc_prog_id;
   END IF;

   fnd_file.put_line(fnd_file.output, '');
   fnd_file.put_line(fnd_file.output, 'Program Name: ' || lv_conc_prog_name);
   fnd_file.put_line(fnd_file.output, 'Request ID:   ' || ln_conc_req_id);
   fnd_file.put_line(fnd_file.output, 'Run Date:     ' || TO_CHAR(SYSDATE, 'YYYY/DD/MM HH24:MI:SS'));
   fnd_file.put_line(fnd_file.output, 'User Name:    ' || lv_user_name);
   fnd_file.put_line(fnd_file.output, '');
   fnd_file.put_line(fnd_file.output, '');
   fnd_file.put_line(fnd_file.output, '============================================================================================================');
   ...
   IF sql%FOUND THEN
      ...
      -- Include information regarding deactivated
      -- Kofax invoice
      FOR i_out IN (SELECT 'Invoice Number:  ' || scx.invoice_num invoice_num,
                           'Amount Excl GST: ' || scx.invoice_amount_exc_gst amount_exc_gst,
                           'GST Amount:      ' || scx.gst_amount gst_amount,
                           'PO Number:       ' || scx.po_number po_number,
                           'Attachment URL:  ' || NVL(scx.po_number_attachment, scx.invoice_id_attachment) attach_url,
                           'Status:          Deactivated' status
                    FROM   xxap_inv_scanned_file scx
                    WHERE  scx.payload_id = p_payload_id)
      LOOP
         fnd_file.put_line(fnd_file.output, i_out.invoice_num);
         fnd_file.put_line(fnd_file.output, i_out.amount_exc_gst);
         fnd_file.put_line(fnd_file.output, i_out.gst_amount);
         fnd_file.put_line(fnd_file.output, i_out.po_number);
         fnd_file.put_line(fnd_file.output, i_out.attach_url);
         fnd_file.put_line(fnd_file.output, i_out.status);
      END LOOP;
      fnd_file.put_line(fnd_file.log, 'Payload ID: ' || p_payload_id || '  Deactivated: Y');

   ELSE
      fnd_file.put_line(fnd_file.output, 'Unable to deactivate Payload ID ' || p_payload_id);
   END IF;
   fnd_file.put_line(fnd_file.output, '============================================================================================================');

EXCEPTION
   WHEN OTHERS THEN
      fnd_file.put_line(fnd_file.log, 'Unexpected error while trying to deactivate ' || SQLERRM);
      p_retcode := 2;
