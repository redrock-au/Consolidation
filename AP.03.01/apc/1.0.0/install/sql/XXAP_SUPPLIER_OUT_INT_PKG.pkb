...
/* $Header: svn://d02584/consolrepos/branches/AP.03.01/apc/1.0.0/install/sql/XXAP_SUPPLIER_OUT_INT_PKG.pkb 3169 2017-12-11 07:42:37Z svnuser $ */
/*******************************************************************
**
** CEMLI ID: AP.02.02
**
...
(
   ...
   CURSOR c_vend(p_date IN DATE) IS
      SELECT pov.segment1 vendor_number,
             ...
             pos.attribute13 rcti_flag,
             DECODE(p_source,'GEMS',
                             pos.payment_method_lookup_code, 
                             pov.payment_method_lookup_code) payment_method, -- FSC-5915 Payment Method shows data from Vendor instead of Site (GEMS) 
             ...
             pos.attribute3 remittance_email,
             GREATEST(pos.last_update_date, pov.last_update_date) last_updated_date,
             NVL2(pos.inactive_date, (CASE WHEN pos.inactive_date < SYSDATE THEN 'Inactive' 
                                           ELSE 'Active' 
                                      END), 'Active') status,
             -- FSC-5974 Supplier outbound file for GEMS : Added Site's Bank Account and BSB by Daniel Kwon 17-11-2017
             (bank.bank_number || bank.bank_num) bsb,
             bank.bank_account_num bank_account
      FROM   po_vendors pov,
             po_vendor_sites pos,
             ...
             AND    ba.bank_branch_id = br.bank_branch_id -- FSC-5974 17-11-2017
             -- FSC-6092 06-DEC-2017 : Return Only Primary Bank Account for the Supplier Site
             AND    bu.primary_flag = 'Y'
             AND    TRUNC(SYSDATE) <= NVL(bu.end_date, TRUNC(SYSDATE + 1))
             ) bank 
      WHERE  pos.vendor_id = pov.vendor_id
      AND    (
             pov.last_update_date >= p_selection_date 
             OR 
