...
/* $Header: svn://d02584/consolrepos/branches/AP.03.01/glc/1.0.0/install/sql/XXGL_PAYROLL_PKG.pkb 2814 2017-10-19 00:05:41Z svnuser $ */
/****************************************************************************
**
** CEMLI ID: GL.02.01
**
...
BEGIN
   ...
   INTO   l_int_rows
   FROM   xxgl_payroll_detail_tfm
   WHERE  reference23 = 'INT'
   AND    run_id = p_run_id;

   IF l_int_rows > 0 THEN
      -- arellanod 14/09/2017 FSC-4166
      -- Remove zero balance Inter-company lines
      DELETE FROM xxgl_payroll_detail_tfm tfd
      WHERE  EXISTS (SELECT a.*
                     FROM   (SELECT run_id,
                             ...
                             GROUP  BY
                                    ...
                                    segment6,
                                    segment7
                             HAVING (NVL(SUM(entered_dr), 0) - NVL(SUM(entered_cr), 0)) = 0
                            ) a
                     WHERE  a.run_id = tfd.run_id
                     AND    a.reference23 = tfd.reference23
                     AND    a.user_je_source_name = tfd.user_je_source_name
                     AND    a.user_je_category_name = tfd.user_je_category_name
                     AND    a.segment1 = tfd.segment1
                     AND    a.segment2 = tfd.segment2
                     AND    a.segment3 = tfd.segment3
                     AND    a.segment4 = tfd.segment4
                     AND    a.segment5 = tfd.segment5
                     AND    a.segment6 = tfd.segment6
                     AND    a.segment7 = tfd.segment7);
   END IF;
...
-- --------------------------------------------------------------------------------------------------
--  PROCEDURE
--  Transformation Summary and Detail Report
--  FSC-3810 : No Payroll reconciliation file generated (after jnl posted)
--  dkwon    
-- --------------------------------------------------------------------------------------------------
PROCEDURE transformation_report
(
   p_run_id              IN   NUMBER
) 
IS
   CURSOR c_summary IS
      /*
      SELECT s.record_category, 
             SUM(nvl(S.amount,0)) HR_amount, 
             T.user_je_category_name, 
             SUM(nvl(T.entered_dr,0)) Journal_maount,
             SUM(nvl(S.amount,0)) - SUM(nvl(T.entered_dr,0))  diferrence
      FROM   xxgl_payroll_detail_tfm T,
             xxgl_payroll_detail_stg S
      WHERE  S.record_id = T.source_record_id(+) 
      AND    (reference23 = 'EXT' OR reference23 IS NULL)
      AND    T.run_id = S.run_id
      AND    T.run_id = p_run_id
      GROUP BY S.record_category, 
               T.user_je_category_name
      ORDER BY user_je_category_name
      */
      SELECT record_category,
             upper(user_je_category_name) user_je_category_name,
             SUM(hr_amount) hr_amount,
             SUM(journal_amount) journal_amount,
             SUM(hr_amount) - SUM(journal_amount) diferrence
      FROM   (SELECT s.record_category record_category,
                     (NVL(s.amount, 0)) hr_amount,
                     t.user_je_category_name,
                     TO_NUMBER(NVL(t.reference26, 0)) journal_amount
              FROM   xxgl_payroll_detail_tfm t,
                     xxgl_payroll_detail_stg s
              WHERE  s.record_id = t.source_record_id(+)
              AND    (reference23 = 'EXT' OR reference23 IS NULL)
              ...
              SELECT record_category,
                     (amount) hr_amount,
                     record_category user_je_category_name,
                     0
              FROM   xxgl_payroll_detail_del
              WHERE  run_id = p_run_id)
      GROUP BY record_category,
               upper(user_je_category_name)
      ORDER BY record_category;
   ...
   CURSOR c_detail IS
      SELECT record_category,
             ...
             dr,
             cr,
             NVL(ext_off_flag, 'EXT') flag
      FROM   (SELECT s.record_category,
                     s.organisation || '-' || 
                     s.account || '-' || 
                     s.cost_centre || '-' || 
                     s.authority || '-' || 
                     ...
                     t.entered_dr dr,
                     t.entered_cr cr,
                     t.reference23 ext_off_flag,
                     t.source_record_id source_record_id
              FROM   xxgl_payroll_detail_stg s,
                     xxgl_payroll_detail_tfm t
              WHERE  s.record_id = t.source_record_id(+)
              AND    t.run_id = s.run_id
              AND    t.run_id = p_run_id
              /*AND (reference23 = 'EXT' OR reference23 IS NULL)*/
              UNION ALL
              SELECT d.record_category,
                     d.organisation || '-' || 
                     d.account || '-' || 
                     d.cost_centre || '-' || 
                     d.authority || '-' || 
                     ...
                     NULL cr,
                     NULL flag,
                     999999999999 source_record_id
              FROM   xxgl_payroll_detail_del d
              WHERE  run_id = p_run_id)
      ORDER BY record_category,
               source_record_id,
               flag;
     
   l_catgory_total        NUMBER := 0;
   l_journal_total        NUMBER := 0;
   l_diferrence_total     NUMBER := 0;
   l_detail_dr_total      NUMBER := 0;   
   l_detail_cr_total      NUMBER := 0;   
   l_wip_category         xxgl_payroll_detail_stg.record_category%TYPE := 'X';
   l_format_mask          VARCHAR2(30);
BEGIN
   fnd_file.put_line(fnd_file.output, LPAD(' ', 60, ' ')||' Payroll Transformation Summary ');
   fnd_file.put_line(fnd_file.output, LPAD(' ', 60, ' ')||' ------------------------------');

   fnd_file.put_line(fnd_file.output, RPAD('Payroll Category', 45, ' ') || 
                                      RPAD('Amount from HR', 29, ' ') || 
                                      RPAD('Journal Category', 37, ' ') ||
                                      RPAD('Journal Amount', 29, ' ') || RPAD('Difference', 20, ' '));

   --fnd_file.put_line(fnd_file.output,RPAD('----------------------',40,' ')||RPAD('-----------------------',31,' ')||RPAD('-----------------------',35,' ')||RPAD('----------------------',32,' ')||RPAD('--------------',20,' '));
   --fnd_file.put_line(fnd_file.output,' ');

   fnd_file.put_line(fnd_file.output,lpad('-', 152, '-'));
   l_format_mask := fnd_currency.get_format_mask(g_sob_currency, 20);

   FOR r_summary IN c_summary
   LOOP
      fnd_file.put_line(fnd_file.output,  RPAD(r_summary.record_category, 50, ' ') ||
                                          RPAD(LPAD(TO_CHAR(r_summary.hr_amount, l_format_mask), 10, ' '), 20, ' ') ||
                                          RPAD(' ', 5, ' ') ||
                                          RPAD(r_summary.user_je_category_name, 35, ' ') ||
                                          RPAD(LPAD(TO_CHAR(r_summary.journal_amount, l_format_mask), 14, ' '), 14, ' ') ||
                                          RPAD(LPAD(TO_CHAR(r_summary.diferrence, l_format_mask), 25, ' '), 25, ' '));
      --l_wip_category := r_summary.record_category;
      l_catgory_total := l_catgory_total + r_summary.HR_amount;
      l_journal_total := l_journal_total + r_summary.journal_amount;
      l_diferrence_total := l_diferrence_total + r_summary.diferrence;
   END LOOP;
   ...
   fnd_file.put_line(fnd_file.output, RPAD('Total:', 30, ' ') ||
                                      LPAD(TO_CHAR(l_catgory_total, l_format_mask), 30, ' ') ||
                                      LPAD(TO_CHAR(l_journal_total, l_format_mask), 64, ' ') ||
                                      LPAD(TO_CHAR(l_diferrence_total, l_format_mask), 25, ' '));
   ...
   fnd_file.put_line(fnd_file.output, ' ');
   fnd_file.put_line(fnd_file.output, ' ');
   fnd_file.put_line(fnd_file.output,  LPAD(' ', 66, ' ') || ' Transformation Report');
   fnd_file.put_line(fnd_file.output,  LPAD('-', 152, '-'));
   
   FOR r_detail IN c_detail LOOP
      IF r_detail.record_category <> nvl(l_wip_category, 'x') THEN
         debug_msg('l_wip_category :' || l_wip_category);
         fnd_file.put_line(fnd_file.output, RPAD('Category : ' || r_detail.record_category, 32, ' '));
         fnd_file.put_line(fnd_file.output, RPAD('Flag ', 10, ' ') || 
                                            RPAD('From', 50, ' ') ||
                                            RPAD('To', 59, ' ') ||
                                            RPAD('DR Amount', 21, ' ') ||
                                            RPAD('CR Amount', 27, ' '));
      ...
      fnd_file.put_line(fnd_file.output, RPAD(r_detail.flag, 10, ' ') ||
                                         RPAD(r_detail.stg_cc, 50, ' ') ||
                                         RPAD(r_detail.tfm_cc, 36, ' ') ||
                                         LPAD(TO_CHAR(r_detail.dr, l_format_mask), 32, ' ') ||
                                         LPAD(TO_CHAR(r_detail.cr, l_format_mask), 52, ' '));

      l_detail_dr_total := l_detail_dr_total +  NVL(r_detail.dr, 0);       
      l_detail_cr_total := l_detail_cr_total +  NVL(r_detail.cr, 0);  
      l_wip_category := r_detail.record_category;
   END LOOP;
   
   fnd_file.put_line(fnd_file.output, ' ');
   fnd_file.put_line(fnd_file.output, LPAD('-----------------------', 129, ' ') ||
                                      LPAD('-----------------------', 23, ' '));

   fnd_file.put_line(fnd_file.output, RPAD('Total:', 90, ' ') || 
                                      LPAD(TO_CHAR(l_detail_dr_total, l_format_mask), 39, ' ') ||
                                      LPAD(TO_CHAR(l_detail_cr_total, l_format_mask), 20, ' '));

   fnd_file.put_line(fnd_file.output, ' ');
   fnd_file.put_line(fnd_file.output,  '                                                           **      End of Report    **            ');
   
END transformation_report;
...
-- --------------------------------------------------------------------------------------------------
-- FSC-3810 : 09/22/2017 : Update NULL value of REFERENCE23 with the value EXT, 
--  if REFERENCE24 IS NOT NULL and OFFSET_FLAG IS NULL
-- --------------------------------------------------------------------------------------------------
PROCEDURE update_missing_reference23(p_run_id  IN  NUMBER)
IS
BEGIN 
   UPDATE xxgl_payroll_detail_tfm
   SET    reference23 = 'EXT'
   WHERE  reference23 IS NULL
   AND    reference24 IS NOT NULL
   AND    offset_flag IS NULL
   AND    run_id = p_run_id;

   --COMMIT;
END update_missing_reference23;
...
BEGIN
   ...
   */
   IF g_run_number IS NULL THEN
      SELECT DECODE(g_accrual_flag,
                    'N', NVL(MAX(r.payroll_runno), 0) + 1,
                    'Y', NVL(MAX(r.payroll_runno), 0)) -- arellanod 13/09/2017 FSC-4082
      INTO   g_run_number
   ...
   */
   ...
   ELSE
      ...
      /*  if REFERENCE24 IS NOT NULL and OFFSET_FLAG IS NULL
      /****************************************************/
      update_missing_reference23(p_run_id);
...
BEGIN
   ...
   IF l_file LIKE 'payrollacc%' THEN
      g_accrual_flag := 'Y';
      g_user_je_source := 'Payroll';  --'Payroll Accrual';  -- arellanod 13/09/2017 FSC-4082
   ELSE
      g_accrual_flag := 'N';
   ...
   */
   ...
   LOOP
      ...
      transformation_report(l_run_id);
   END LOOP;
