...
/* $Header: svn://d02584/consolrepos/branches/AP.03.01/fndc/1.0.0/install/sql/XXINT_COMMON_PKG.pkb 2756 2017-10-10 04:17:20Z svnuser $ */
...
IS
   ...
   l_default_ap_source   VARCHAR2(25) := 'APCORP';
   l_default_ar_source   VARCHAR2(25) := 'ARCORP';
   l_receipt_source      VARCHAR2(30);
...
BEGIN
   ...
   IF (l_instance_name = 'CONDEV' OR p_source = 'CONV') THEN
         ...
         ELSIF p_application = 'AR' THEN
            BEGIN
               ...
               INTO   l_source,
                      l_receipt_source
               FROM   fnd_lookup_values_vl v,
                      fnd_lookup_types_vl t,
                      fnd_application a
               WHERE  v.lookup_type = 'XXAR_BATCH_SOURCES'
               AND    v.lookup_type = t.lookup_type
               AND    t.application_id = a.application_id
               AND    a.application_short_name = 'ARC'
               AND    v.enabled_flag = 'Y'
               AND    v.lookup_code = p_source
               AND    SYSDATE < NVL(v.end_date_active, SYSDATE + 1);
            EXCEPTION
               WHEN no_data_found THEN
                  l_source := l_default_ar_source;
            END;

            IF xxint_common_pkg.g_object_type = 'RECEIPTS' THEN
               IF l_receipt_source IS NULL THEN
                  l_object := LOWER(xxint_common_pkg.g_object_type);
               ELSE
                  l_object := LOWER(l_receipt_source);
               END IF;
            END IF;
         ELSE
            l_source := p_source;
...
RETURN NUMBER
AS LANGUAGE JAVA
NAME 'file_util.delete (java.lang.String) return java.lang.int';
