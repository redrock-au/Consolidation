...
/* $Header: svn://d02584/consolrepos/branches/AP.03.01/fndc/1.0.0/install/sql/DOT_COMMON_INT_PKG.pkb 3143 2017-12-07 05:34:25Z svnuser $*/
...
is
  ...
  -- FSC-5916 05-DEC-2017 Daniel Kwon
  -- Check whether the Concurrent Program on this Run ID listed in the Lookup XXFND_SEND_NOTIFICATION
  -- If the Interface Program listed in the lookup and enabled, send Notification
  -- If the Interface Program NOT LISTED in the lookup, send Notification as before
  -- If the Interface Program listed in the lookup but DISABLED explicitly, Do not send Notification
    
  CURSOR c_block_sending_noti(this_run_id IN NUMBER) IS
  SELECT distinct concurrent_program_name, 
                ctrl.ENABLED_FLAG,
                ctrl.start_date_active,
                ctrl.end_date_active
  FROM fnd_concurrent_programs pg,
       fnd_concurrent_requests pr,
       dot_int_run_phases run,
       fnd_lookup_values ctrl
  WHERE pr.concurrent_program_id = pg.concurrent_program_id
  AND run.request_id = pr.request_id
  AND run.run_id = this_run_id
  AND run.run_phase_id = (SELECT MAX(run_phase_id) 
                          FROM dot_int_run_phases 
                          WHERE run_id = this_run_id)
  AND ctrl.lookup_type ='XXFND_SEND_NOTIFICATION'
  AND ctrl.lookup_code = pg.concurrent_program_name;
  --AND trunc(sysdate) between start_date_active and nvl(end_date_active,trunc(sysdate));
  
  cv_proc_name constant varchar2(25) := 'launch_run_report';
  v_setup_count         NUMBER :=0;
...
BEGIN
  ...
  -- FSC-5916 05-DEC-2017 Daniel Kwon
  BEGIN 
     FOR r_block_sending_noti IN c_block_sending_noti(p_run_id) LOOP
        IF trunc(sysdate) between r_block_sending_noti.start_date_active and nvl(r_block_sending_noti.end_date_active,trunc(sysdate)) THEN
           IF r_block_sending_noti.ENABLED_FLAG = 'N' THEN
              debug('WF Notification will not be sent. The Notification for this Program '||r_block_sending_noti.concurrent_program_name ||' has been disabled in XXFND_SEND_NOTIFICATION Lookup ', cv_proc_name);
           ELSE
              -- Set the user to be notified
              dot_common_int_pkg.add_notification (pv_user_list => p_notify_user);
              debug('WF Notification was sent to : '||p_notify_user, cv_proc_name);
           END IF;
        ELSE
           debug( 'WF Notification will not be sent. The program '||r_block_sending_noti.concurrent_program_name||' was out of effective date period.', cv_proc_name);
           debug( 'Start Effective : '||r_block_sending_noti.start_date_active, cv_proc_name);
           debug( 'End Effective : '||r_block_sending_noti.end_date_active, cv_proc_name);
        END IF;
        v_setup_count := c_block_sending_noti%ROWCOUNT;        
     END LOOP;
  ...
  EXCEPTION
     ...
     -- Set the user to be notified
           dot_common_int_pkg.add_notification (pv_user_list => p_notify_user);
           debug('The Program of the Run ID not listed in XXFND_SEND_NOTIFICATION Lookup', cv_proc_name);
           debug('WF Notification was sent to : '||p_notify_user, cv_proc_name);
  END;
