...
/* $Header: svn://d02584/consolrepos/branches/AP.00.02/glc/1.0.0/install/sql/XXGL_CASH_BAL_PKG.pkb 1470 2017-07-05 00:33:23Z svnuser $ */
...
z_revenue_acct      CONSTANT VARCHAR2(3)   := 'REV';
z_receivable_acct   CONSTANT VARCHAR2(3)   := 'REC';
...
BEGIN
   l_date := ADD_MONTHS(TRUNC(SYSDATE), -12);
   l_sql := 'TRUNCATE TABLE FMSMGR.XXGL_CASH_BAL_STG';
...
IS
   l_source             gl_je_headers.je_source%TYPE;
...
BEGIN
   ...
   SELECT ABS(NVL(jel.entered_dr, 0) - NVL(jel.entered_cr, 0))
   INTO   l_jel_amount
   ...
   /* Debug */
   print_debug(p_sl_trx_type || ' ' || p_sl_trx_subtype);
      ...
      ELSIF p_sl_trx_type = 'TRADE' AND p_sl_trx_subtype = 'TRADE_REC' THEN
         ...
         BEGIN
            ...
            FROM   (SELECT ll.line_number,
                    ...
                    WHERE  ll.customer_trx_id = (SELECT ar.applied_customer_trx_id
            ...
            GROUP  BY x.code_combination_id;
   ...
   ELSIF p_source = 'Payables' THEN
      IF p_sl_trx_type = 'INVOICE'  AND p_sl_trx_subtype = 'LIABILITY' THEN 
         ...
         SELECT '' line,
                ...
                SUM(DECODE(NVL(t.tax_rate, 0), 0, l.amount, ((t.tax_rate / 100) + 1) * l.amount)) amount
         FROM   ap_invoice_distributions_all l,
                ap_invoices_all i,
                ap_tax_codes_all t
         WHERE  l.org_id = g_org_id
         AND    l.invoice_id = i.invoice_id
         AND    i.invoice_amount <> 0
         AND    l.tax_code_id = t.tax_id(+)
         AND    l.line_type_lookup_code <> 'TAX'
         AND    l.invoice_id IN (SELECT aala.source_id
                                 FROM   ap_ae_headers_all aaha,
                                        ap_ae_lines_all aala,
                                        gl_je_lines jel
                                 ...
                                 AND    jel.je_header_id = p_je_header_id
                                 AND    jel.je_line_num = p_je_line_num)
         GROUP  BY l.dist_code_combination_id;

      ELSIF p_sl_trx_type = 'INVOICE'  AND p_sl_trx_subtype = 'RECOVERABLE TAX' THEN 
         OPEN c_dist_sum;
   ...
   /* Debug */
   print_debug('distribution_lines=' || p_sl_tab.COUNT);
...
BEGIN
   ...
   SET    status = p_process_status
   WHERE  group_id = p_group_id;

   IF p_process_status = 'SUCCESS' THEN
      ...
      WHERE  jeh.je_source = sou.je_source_name
      AND    jeh.je_category = cat.je_category_name
      AND    jeh.period_name = g_period_name -- utilises index
      AND    jeh.set_of_books_id = g_sob_id  -- utilises index
      AND    EXISTS (SELECT 1
                     FROM   gl_je_lines jel
                     WHERE  jel.je_header_id = jeh.je_header_id
                     AND    jel.set_of_books_id = jeh.set_of_books_id -- utilises index
                     AND    jel.period_name = jeh.period_name         -- utilises index
                     AND    jel.reference_4 = TO_CHAR(p_group_id));
   END IF;
...
IS
   CURSOR c_jel IS
      SELECT jel.je_header_id,
             ...
             ((NVL(jel.entered_dr, 0) - NVL(jel.entered_cr, 0)) * DECODE(enter_amount, '-', -1, 1)) amount
      FROM   gl_je_lines jel,
             gl_je_headers jeh,
             gl_je_batches jeb,
         ...
         IF t_sl_tab.COUNT > 0 AND l_message IS NULL THEN
            FOR i IN 1 .. t_sl_tab.COUNT LOOP
               ...
               VALUES(xxgl_cash_bal_record_id_s.NEXTVAL,
                      ...
                      DECODE(r_jel.entity_offset_flag, z_yes_flag, t_sl_tab(i).segment1, r_jel.segment1),
                      r_jel.segment2,
                      DECODE(r_jel.default_value_flag, z_yes_flag, t_sl_tab(i).segment3, r_jel.segment3),
                      DECODE(r_jel.default_value_flag, z_yes_flag, t_sl_tab(i).segment4, r_jel.segment4),
                      DECODE(r_jel.default_value_flag, z_yes_flag, t_sl_tab(i).segment5, r_jel.segment5),
                      DECODE(r_jel.default_value_flag, z_yes_flag, t_sl_tab(i).segment6, r_jel.segment6),
                      DECODE(r_jel.default_value_flag, z_yes_flag, t_sl_tab(i).segment7, r_jel.segment7),
                      DECODE(SIGN(r_jel.amount), 1, t_sl_tab(i).amount, NULL),
                      DECODE(SIGN(r_jel.amount), 1, NULL, t_sl_tab(i).amount),
                      r_jel.summary_flag);
            END LOOP;
         ELSE
         ...
         BEGIN
            ...
            IF l_gst_flag = 'Y' THEN
               ...
               VALUES (xxgl_cash_bal_record_id_s.NEXTVAL,
                       ...
                       r_jel.segment6,
                       r_jel.segment7,
                       DECODE(SIGN(r_jel.amount), 1, l_gst_amount, NULL),
                       DECODE(SIGN(r_jel.amount), 1, NULL, l_gst_amount),
                       r_jel.summary_flag);
            END IF;
         EXCEPTION
            WHEN no_data_found THEN
...
BEGIN
   ...
   /* Debug */
   print_debug('=======================================================');
   print_debug('batch_id=' || p_je_batch_id);
   ...
   LOOP
      ...
      BEGIN
         ...
         IF l_exists_count > 0 AND NVL(p_force_flag, 'N') = 'N' THEN
            RAISE e_posted_journal;
         END IF;

         IF r_batch.status = z_posted AND NVL(p_force_flag, 'N') = 'N' THEN
            RAISE e_posted_journal;
         END IF;
   ...
   IF l_run_status = 'APPLY' THEN
      ...
      -- Load  --
      -----------
      fnd_file.put_line(fnd_file.log, 'tfm_error_count=' || l_tfm_error_count);
      ...
      SELECT sl_source,
             ...
             CASE
                ...
                WHEN sl_source = 'Payables' THEN
                   DECODE(summary_flag, 'N', 
                          token_name || ' (' || source_je_header_id || '-' || source_je_line_num || '-' || rule_num || ')',
                          token_name || ' (' || source_je_header_id || '-0-' || rule_num || ')')
             END description,
             rule_num,
             segment1,
...
BEGIN
   ...
   LOOP
      ...
      VALUES (p_request_id,
              ...
              NULL,                                  --reference24 (use for group_id)
              NULL,                                  --reference25
              NULL,                                  --summary_flag
              l_status,
              l_message,
   ...
   LOOP
      ...
      IF (NVL(r_tfm.entered_dr, 0) + NVL(r_tfm.entered_cr, 0) <> 0) THEN
   ...
   IF run_merge THEN
      ...
      LOOP
         ...
         IF r_stg.org_id <> TO_CHAR(l_org_id) THEN
            fnd_file.put_line(fnd_file.log, z_error || r_stg.rule_num || ' - expectting ORG_ID ' || l_org_id || '.');
            l_status := 'ERROR';
         END IF;
