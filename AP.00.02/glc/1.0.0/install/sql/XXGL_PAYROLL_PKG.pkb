create or replace package body xxgl_payroll_pkg
as
/* $Header: svn://d02584/consolrepos/branches/AP.00.02/glc/1.0.0/install/sql/XXGL_PAYROLL_PKG.pkb 1470 2017-07-05 00:33:23Z svnuser $ */
g_user_id                NUMBER;
...
BEGIN
   ...
   LOOP
      ...
      IF l_update_sql IS NOT NULL THEN
         -- Build SQL Clause
         l_sql := 'update ' || p_target_table || ' set ';
         IF t_rules_tab(i).lookup_code LIKE '%OFF0' THEN
            -- Offset rows are flagged as such and have their amounts reversed
            -- If a rule is split over more than one rule then do this only on the first rule
            -- (e.g. do it for R09OFF0 but not R09OFF1)
            -- Otherwise the entered_cr and entered_dr will be switched back
            l_sql := l_sql || 'offset_flag = ''Y'', entered_dr = entered_cr, entered_cr = entered_dr ';
         END IF;
         -- Append configured Update clause
         IF t_rules_tab(i).lookup_code LIKE '%OFF0' THEN
            l_sql := l_sql || ', ';
         END IF;
         -- Special logic for date/run sequence substitution
         l_update_sql := replace(l_update_sql, 'YYYYXX', p_year || p_pay_num);
...
BEGIN
   ...
   */
   ...
   ELSE
      ...
      /****************************************************/
      apply_journal_rules(p_run_id, 'EXT', p_accrual_option, g_rule_lookup_type, 'XXGL_PAYROLL_DETAIL_TFM', l_period_year, l_pay_count + 1);
      /****************************************************/
      /* Apply Offset (OFF) Rules to the temporary table  */
      /* then insert them into the TFM                    */
      /****************************************************/
      apply_journal_rules(p_run_id, 'OFF', p_accrual_option, g_rule_lookup_type, 'XXGL_PAYROLL_DETAIL_TMP', l_period_year, l_pay_count + 1);
      insert_offset_from_temp;
      /****************************************************/
