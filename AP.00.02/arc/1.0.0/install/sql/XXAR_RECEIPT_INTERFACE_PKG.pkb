...
/* $Header: svn://d02584/consolrepos/branches/AP.00.02/arc/1.0.0/install/sql/XXAR_RECEIPT_INTERFACE_PKG.pkb 1470 2017-07-05 00:33:23Z svnuser $ */
/*******************************************************************
**
** CEMLI ID: AR.02.03
**
...
--      is_payment_notice
--  PURPOSE
--      Determines whether the CRN is a custom payment notice.
--  RETURNS
--      True if it is, else False
-- --------------------------------------------------------------------------------------------------
FUNCTION is_payment_notice
(
   p_crn                   VARCHAR2
) RETURN BOOLEAN
IS
...
BEGIN
   ...
   */
   IF is_payment_notice(p_crn) THEN
...
BEGIN
   ...
   ELSE
      IF p_source = 'RECALL' THEN
         -- recall receipt numbers are zero left-padded numeric values
         -- strip off the leading zeros
         p_tfm_rec.receipt_number := to_char(to_number(p_receipt_number));
      ELSE
         p_tfm_rec.receipt_number := p_receipt_number;
      END IF;
   END IF;
