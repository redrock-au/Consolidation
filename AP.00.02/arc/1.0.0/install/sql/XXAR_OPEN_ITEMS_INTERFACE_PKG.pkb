...
/* $Header: svn://d02584/consolrepos/branches/AP.00.02/arc/1.0.0/install/sql/XXAR_OPEN_ITEMS_INTERFACE_PKG.pkb 1155 2017-06-22 23:04:38Z svnuser $ */
...
--    GET_OBJECT_VERSION
-- Purpose
--    Add a version number to be able to trace history
--    of a transaction record. Use the version object
--    version number in lieu of accounting period.
-------------------------------------------------------
...
BEGIN
   ...
   FOR i IN 1 .. t_files.COUNT LOOP
      ...
      LOOP
         ...
         r_error.int_table_key_val1 := r_stg.crn_number;
         ...
         IF NVL(p_archive_flag, 'N') = 'N' THEN
            ...
            INTO   l_dup
            FROM   xxar_payment_notices_b
            WHERE  crn_number = r_stg.crn_number
      ...
      -- Exit loop regardless of the
      -- number of files fetched.
      -- Business rule is to process only
...
--     and extracts data into the prescribe bank upload
--     format.
----------------------------------------------------------
