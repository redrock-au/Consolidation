...
/* $Header: svn://d02584/consolrepos/branches/AP.00.02/arc/1.0.0/install/sql/XXAR_INVOICES_INTERFACE_PKG.pkb 1470 2017-07-05 00:33:23Z svnuser $ */
...
--     GENERATE_CRN
-- Purpose
--     Builds the Customer Reference Number printed on
--     the the tax invoice.
-------------------------------------------------------
...
IS
   ...
   l_counter            NUMBER := 1;
   l_offset_length      NUMBER;
   l_customer_trx_id    VARCHAR2(30) := p_customer_trx_id;
...
BEGIN
   ...
   -- At this stage expected length is always 11. It will
   -- never be more or less than 11 digits.
   l_counter := 1;
...
BEGIN
   ...
   LOOP
      ...
      AND    tfm.trx_number = t_load(i).trx_number
   ...
   LOOP
      ...
      ELSE
         BEGIN
            ...
            IF (LENGTH(r_tfm.unit_selling_price) - INSTR(r_tfm.unit_selling_price, '.')) > 2 THEN
               calculate_amount := FALSE;
               l_tfm_error := l_tfm_error + 1;
      ...
      ELSE
         BEGIN
            IF (TO_NUMBER(r_stg.amount) > 0) AND calculate_amount THEN
               r_tfm.amount := (TO_NUMBER(r_stg.quantity) * TO_NUMBER(r_stg.unit_selling_price));
               IF ROUND(r_tfm.amount, 2) <> ROUND(TO_NUMBER(r_stg.amount), 2) THEN
                  l_tfm_error := l_tfm_error + 1;
