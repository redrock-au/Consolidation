create or replace PACKAGE BODY POC_DOC_NOTIFICATION AS
/* $Header: svn://d02584/consolrepos/branches/AP.00.02/poc/1.0.0/install/sql/POC_DOC_NOTIFICATION.pkb 1470 2017-07-05 00:33:23Z svnuser $ */

Function   PrintDocument(itemtype varchar2,itemkey varchar2) return varchar2;
...
BEGIN
  ...
  resultout := wf_engine.eng_completed || ':' || PrintDocument(itemtype,itemkey);
  --
...
Function   PrintDocument(itemtype varchar2,itemkey varchar2) return varchar2 is
...
if funcmode = 'RUN' then
   ...
   x_request_id  := wf_engine.GetItemAttrNumber (itemtype => itemtype,
                                                 itemkey  => itemkey,
                                                 aname    => 'CONCURRENT_REQUEST_ID');
   x_request_id2  := wf_engine.GetItemAttrNumber (itemtype => itemtype,
                                                 itemkey  => itemkey,
                                                 aname    => 'CONCURRENT_REQUEST_ID2');
   x_timeout_cnt := wf_engine.GetItemAttrNumber (itemtype => itemtype,
                                                 itemkey  => itemkey,
                                                 aname    => 'TIMEOUT_CNT');
