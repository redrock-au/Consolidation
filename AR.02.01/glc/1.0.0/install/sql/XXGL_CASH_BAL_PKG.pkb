...
/* $Header: svn://d02584/consolrepos/branches/AR.02.01/glc/1.0.0/install/sql/XXGL_CASH_BAL_PKG.pkb 2981 2017-11-14 08:13:57Z svnuser $ */
...
BEGIN
   ...
   IF p_source = 'Receivables' THEN
      ...
      ELSIF p_sl_trx_type = 'TRADE' AND p_sl_trx_subtype IN ('TRADE_REC', 'TRADE_CASH') THEN -- Added TRADE_CASH FSC-5206
...
IS
   ...
   c_stg_cursor         cursor_type;
   t_sl_tab             t_sl_tab_type;
   t_alloc_tab          t_alloc_tab_type;
   r_stage_line         r_stage_line_rec_type;
   r_jel                c_jel%ROWTYPE;
   ...
   l_rec_type           NUMBER;
   l_stg_sql            VARCHAR2(6000);
   l_record_ids         VARCHAR2(1000);
 --l_idx                NUMBER := 0;
BEGIN
   l_stg_sql := 'SELECT record_id, ' ||
                '(NVL(entered_dr, 0) - NVL(entered_cr, 0)) amount, ' ||
                'DECODE(SIGN(NVL(entered_dr, 0) - NVL(entered_cr, 0)), 1, ''DR'', ''CR'') dr_cr ' ||
   ...
   LOOP
      ...
      /*
      IF t_alloc_tab.COUNT > 0 THEN
         FOR i IN 1 .. t_alloc_tab.COUNT LOOP
            IF t_alloc_tab(i).je_header_id = r_jel.je_header_id AND
               t_alloc_tab(i).je_line_num = r_jel.je_line_num AND
               t_alloc_tab(i).rule_num = r_jel.rule_num THEN
               GOTO next_record;
            END IF;
         END LOOP;
      END IF;
      ...
      l_idx := l_idx + 1;
      t_alloc_tab(l_idx).je_header_id := r_jel.je_header_id;
      t_alloc_tab(l_idx).je_line_num := r_jel.je_line_num;
      t_alloc_tab(l_idx).rule_num := r_jel.rule_num;
      */
      ...
      IF r_jel.rule_type = 'CSH' THEN
         ...
         IF t_sl_tab.COUNT > 0 AND l_message IS NULL THEN
            ...
            -- <Start of FSC-5899>
            l_calc_amount := (l_dr_tot - l_cr_tot);
            l_line_amount := ABS(r_jel.amount);

            IF l_calc_amount <> 0 THEN
               l_line_amount := l_line_amount * SIGN(l_calc_amount);
               l_diff_amount := (l_calc_amount - l_line_amount);

               IF NVL(l_diff_amount, 0) <> 0 THEN
                  l_stg_sql := REPLACE(l_stg_sql, '$REQUEST_ID', p_request_id);
                  l_stg_sql := REPLACE(l_stg_sql, '$RECORD_ID', l_record_ids);

                  IF SIGN(l_diff_amount) = 1 THEN
                     l_stg_sql := l_stg_sql || 'DESC';
                  END IF;

                  OPEN c_stg_cursor FOR l_stg_sql;
                  LOOP
                     FETCH c_stg_cursor INTO r_stage_line;
                     EXIT WHEN c_stg_cursor%NOTFOUND;

                     l_new_amount := r_stage_line.amount - l_diff_amount;

                     UPDATE xxgl_cash_bal_stg
                     SET    entered_dr = DECODE(entered_dr, NULL, NULL, ABS(l_new_amount)),
                            entered_cr = DECODE(entered_cr, NULL, NULL, ABS(l_new_amount))
                     WHERE  record_id = r_stage_line.record_id;

                     /*
                     print_debug('Record ID: ' || r_stage_line.record_id);
                     print_debug('Calculated Amount: ' || l_calc_amount);
                     print_debug('Journal Line Amount: ' || l_line_amount);
                     print_debug('Stage Amount: ' || r_stage_line.amount);
                     print_debug('Rounded Amount: ' || l_new_amount);
                     */
