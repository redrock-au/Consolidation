...
/* $Header: svn://d02584/consolrepos/branches/AP.02.02/arc/1.0.0/install/sql/XXAR_PRINT_INVOICES_PKG.pkb 3150 2017-12-07 22:14:52Z svnuser $ */
/******************************************************************************
**
**
   ...
   */
   ...
   IF p_print_command_line = 'Y' AND
      ...
      FOR r_prt IN c_prt_line (ln_this_request) LOOP
        ...
        -- FSC-6016 : Check Email Information to decide whether it will print or not
         BEGIN
           SELECT COUNT(*)
           INTO lv_mail_count
         ...
         END;
         IF lv_mail_count > 0 AND  p_email_function = 'Y' THEN  
         -- FSC-6016 : If the invoice has Email information and Parameter p_email_function has Y then do not print  
            NULL;
            fnd_file.put_line(fnd_file.log,'Mail Count :'||lv_mail_count);
            fnd_file.put_line(fnd_file.log,'DEDJTR Print Command Line will NOT be running');
         ELSE
            fnd_file.put_line(fnd_file.log,'**Print Start');
            b_print := FND_REQUEST.SET_PRINT_OPTIONS(copies => 0);
